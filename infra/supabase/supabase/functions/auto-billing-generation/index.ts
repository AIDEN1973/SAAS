  ...
  try {
    ...
    try {
      await createExecutionAuditRecord(supabase, {
        ...
        operation_type: 'scheduler.auto-billing-generation',
        status: errors.length > 0 ? 'partial' : 'success',
        source: 'scheduler', // 스케줄러 실행 (액티비티.md 6.1 참조)
        actor_type: 'system',
        actor_id: 'svc:edge:auto-billing-generation',
        summary: `청구서 자동 생성 배치 작업 완료 (${totalGenerated}건 생성)`,
        details: {
          generated_count: totalGenerated,
          tenant_count: tenants?.length || 0,
          execution_period: currentMonth,
          errors: errors.length > 0 ? errors : undefined,
        },
        reference: {
          entity_type: 'batch',
          entity_id: `auto-billing-generation:${currentMonth}`,
          source_event_id: `scheduler:auto-billing-generation:${currentMonth}:${Date.now()}`,
        },
    ...
    try {
      ...
      await createExecutionAuditRecord(supabaseForAudit, {
        ...
        actor_type: 'system',
        actor_id: 'svc:edge:auto-billing-generation',
        summary: `청구서 자동 생성 배치 작업 실패`,
        details: null,
        reference: {
          entity_type: 'batch',
          entity_id: `auto-billing-generation:${toKST().toISOString().substring(0, 7)}`,
        },
