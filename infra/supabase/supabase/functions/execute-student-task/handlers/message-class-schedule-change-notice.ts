...
/**
 * 메시지/공지: 수업 일정 변경 안내 Handler
 *
 * Intent: message.exec.class_schedule_change_notice
...
export const messageClassScheduleChangeNoticeHandler: IntentHandler = {
  ...
  async execute(
    ...
    try {
      ...
      try {
        ...
        console.error('[messageClassScheduleChangeNoticeHandler] Invalid event_type:', maskedError);
        return {
          status: 'failed',
      ...
      if (studentsError) {
        ...
        console.error('[messageClassScheduleChangeNoticeHandler] Failed to fetch students:', maskedError);
        return {
          status: 'failed',
        ...
        if (primaryGuardian) {
          ...
          if (recipient) {
            notifications.push({
              ...
              content: plan.plan_snapshot.summary || '수업 일정이 변경되었습니다.',
              status: 'pending',
      ...
      if (notificationError) {
        ...
        console.error('[messageClassScheduleChangeNoticeHandler] Failed to create notifications:', maskedError);
        return {
          status: 'failed',
      ...
      return {
        ...
        message: `${affectedCount}명에게 수업 일정 변경 안내를 발송했습니다.`,
      };
    } catch (error) {
      const maskedError = maskPII(error);
      console.error('[messageClassScheduleChangeNoticeHandler] Execution failed:', maskedError);
      return {
        status: 'failed',
