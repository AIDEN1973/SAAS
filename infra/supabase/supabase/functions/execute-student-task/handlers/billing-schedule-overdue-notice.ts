...
/**
 * 수납/청구: 연체 안내 예약 발송 Handler
 *
 * Intent: billing.exec.schedule_overdue_notice
...
export const billingScheduleOverdueNoticeHandler: IntentHandler = {
  ...
  async execute(
    ...
    try {
      ...
      try {
        ...
        console.error('[billingScheduleOverdueNoticeHandler] Invalid event_type:', maskedError);
        return {
          status: 'failed',
      ...
      if (invoicesError) {
        ...
        console.error('[billingScheduleOverdueNoticeHandler] Failed to fetch invoices:', maskedError);
        return {
          status: 'failed',
        ...
        if (primaryGuardian && primaryGuardian.phone) {
          ...
          if (!guardianNotifications.has(key)) {
            guardianNotifications.set(key, {
              ...
              content: plan.plan_snapshot.summary || `[${month}] 연체 안내`,
              status: 'scheduled',
              scheduled_at: sendAt,
      ...
      if (notificationError) {
        ...
        console.error('[billingScheduleOverdueNoticeHandler] Failed to create notifications:', maskedError);
        return {
          status: 'failed',
      ...
      return {
        ...
        message: `${affectedCount}명에게 연체 안내를 예약했습니다.`,
      };
    } catch (error) {
      const maskedError = maskPII(error);
      console.error('[billingScheduleOverdueNoticeHandler] Execution failed:', maskedError);
      return {
        status: 'failed',
