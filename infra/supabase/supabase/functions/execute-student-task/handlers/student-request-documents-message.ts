...
/**
 * 학생: 문서 요청 메시지 발송 Handler
 *
 * Intent: student.exec.request_documents_message
...
export const studentRequestDocumentsMessageHandler: IntentHandler = {
  ...
  async execute(
    ...
    try {
      ...
      const eventType = plan.event_type || 'new_member_drop';
      ...
      try {
        ...
        console.error('[studentRequestDocumentsMessageHandler] Invalid event_type:', maskedError);
        return {
          status: 'failed',
      ...
      if (guardianError || !guardian) {
        ...
        console.error('[studentRequestDocumentsMessageHandler] Failed to fetch guardian:', maskedError);
        return {
          status: 'failed',
          error_code: 'TARGET_NOT_FOUND',
      ...
      const notification = {
        ...
        content: plan.plan_snapshot.summary || '필요한 문서를 제출해주세요.',
        status: 'pending',
      ...
      if (notificationError) {
        ...
        console.error('[studentRequestDocumentsMessageHandler] Failed to create notification:', maskedError);
        return {
          status: 'failed',
      ...
      return {
        ...
        affected_count: 1,
        message: '문서 요청 메시지 발송을 완료했습니다.',
      };
    } catch (error) {
      const maskedError = maskPII(error);
      console.error('[studentRequestDocumentsMessageHandler] Execution failed:', maskedError);
      return {
        status: 'failed',
