...
/**
 * 출결: 결석 대상 보호자 알림 발송 Handler
 *
 * Intent: attendance.exec.notify_guardians_absent
...
export const attendanceNotifyGuardiansAbsentHandler: IntentHandler = {
  ...
  async execute(
    ...
    try {
      ...
      try {
        ...
        console.error('[attendanceNotifyGuardiansAbsentHandler] Invalid event_type:', maskedError);
        return {
          status: 'failed',
      ...
      if (guardiansError) {
        ...
        console.error('[attendanceNotifyGuardiansAbsentHandler] Failed to fetch guardians:', maskedError);
        return {
          status: 'failed',
      ...
      const notifications = guardianContacts.map((contact) => ({
        ...
        content: plan.plan_snapshot.summary || '결석 안내',
        status: 'pending',
      ...
      if (notificationError) {
        ...
        console.error('[attendanceNotifyGuardiansAbsentHandler] Failed to create notifications:', maskedError);
        return {
          status: 'failed',
      ...
      return {
        ...
        message: `${affectedCount}명에게 결석 알림을 발송했습니다.`,
      };
    } catch (error) {
      // ⚠️ P0: PII 마스킹 필수 (체크리스트.md 4. PII 마스킹)
      const maskedError = maskPII(error);
      console.error('[attendanceNotifyGuardiansAbsentHandler] Execution failed:', maskedError);
      return {
        status: 'failed',
