...
/**
 * 출결: 결석 사유 요청 메시지 발송 Handler
 *
 * Intent: attendance.exec.request_reason_message
...
export const attendanceRequestReasonMessageHandler: IntentHandler = {
  ...
  async execute(
    ...
    try {
      ...
      try {
        ...
        console.error('[attendanceRequestReasonMessageHandler] Invalid event_type:', maskedError);
        return {
          status: 'failed',
      ...
      if (guardiansError) {
        ...
        console.error('[attendanceRequestReasonMessageHandler] Failed to fetch guardians:', maskedError);
        return {
          status: 'failed',
      ...
      const notifications = guardianContacts.map((contact) => ({
        ...
        content: plan.plan_snapshot.summary || '결석 사유 요청',
        status: 'pending',
        event_type: 'absence_first_day',
        execution_context: {
      ...
      if (notificationError) {
        ...
        console.error('[attendanceRequestReasonMessageHandler] Failed to create notifications:', maskedError);
        return {
          status: 'failed',
      ...
      return {
        ...
        message: `${affectedCount}명에게 결석 사유 요청 메시지를 발송했습니다.`,
      };
    } catch (error) {
      const maskedError = maskPII(error);
      console.error('[attendanceRequestReasonMessageHandler] Execution failed:', maskedError);
      return {
        status: 'failed',
