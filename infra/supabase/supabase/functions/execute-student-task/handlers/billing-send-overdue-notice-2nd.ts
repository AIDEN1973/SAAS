...
/**
 * 수납/청구: 2차 연체 안내 발송 Handler
 *
 * Intent: billing.exec.send_overdue_notice_2nd
...
export const billingSendOverdueNotice2ndHandler: IntentHandler = {
  ...
  async execute(
    ...
    try {
      ...
      try {
        ...
        console.error('[billingSendOverdueNotice2ndHandler] Invalid event_type:', maskedError);
        return {
          status: 'failed',
      ...
      if (invoicesError) {
        ...
        console.error('[billingSendOverdueNotice2ndHandler] Failed to fetch invoices:', maskedError);
        return {
          status: 'failed',
        ...
        if (primaryGuardian && primaryGuardian.phone) {
          ...
          if (!guardianNotifications.has(key)) {
            guardianNotifications.set(key, {
              ...
              content: plan.plan_snapshot.summary || `[${month}] 2차 연체 안내`,
              status: 'pending',
      ...
      if (notificationError) {
        ...
        console.error('[billingSendOverdueNotice2ndHandler] Failed to create notifications:', maskedError);
        return {
          status: 'failed',
      ...
      return {
        ...
        message: `${affectedCount}명에게 2차 연체 안내를 발송했습니다.`,
      };
    } catch (error) {
      const maskedError = maskPII(error);
      console.error('[billingSendOverdueNotice2ndHandler] Execution failed:', maskedError);
      return {
        status: 'failed',
