-- Fix: Replace ALL sc.status references with sc.is_active in automation triggers
-- [문제] student_classes 테이블에는 status 컬럼이 없고 is_active 컬럼만 존재
-- [해결] 모든 sc.status 참조를 sc.is_active로 수정

-- 1. 기존 트리거 삭제 (있다면)
DROP TRIGGER IF EXISTS notify_guardians_on_class_change_trigger ON public.academy_classes;

-- 2. 트리거 함수 재생성 (수정된 버전)
CREATE OR REPLACE FUNCTION public.notify_guardians_on_class_change()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
-- 3. 트리거 재생성
CREATE TRIGGER notify_guardians_on_class_change_trigger
  AFTER UPDATE OR DELETE ON public.academy_classes
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_guardians_on_class_change();

COMMENT ON FUNCTION public.notify_guardians_on_class_change IS
'[수정됨] student_classes.status → student_classes.is_active로 변경 (162번 migration)';
