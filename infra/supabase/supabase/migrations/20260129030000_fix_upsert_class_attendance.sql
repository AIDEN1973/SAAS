/**
 * Migration: Fix upsert_attendance_log for class attendance records
 *
 * [문제]
 * 1. upsert_attendance_log 함수가 attendance_type=null인 수업 출석 레코드를 처리하지 못함
 * 2. 기존 레코드 검색 조건에 class_id가 포함되지 않아 중복 레코드 생성됨
 * 3. excused 저장 후 새로고침하면 absent로 표시되는 문제
 *
 * [해결]
 * 1. attendance_type 검증을 nullable로 변경
 * 2. class_id 기반 기존 레코드 검색 조건 추가
 * 3. 이중 레코드 패턴 지원:
 *    - check_in/check_out 이벤트: attendance_type != null, class_id = null
 *    - 수업 출석 레코드: attendance_type = null, class_id != null
 */
...
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE '✅ upsert_attendance_log 함수 업데이트 완료 (이중 레코드 패턴 지원)';
  ELSE
