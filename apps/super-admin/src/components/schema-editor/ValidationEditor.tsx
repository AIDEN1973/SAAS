...
export function ValidationEditor({ field, onChange }: ValidationEditorProps) {
  ...
  const handleChange = (key: keyof NonNullable<FormFieldSchema['validation']>, value: unknown) => {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-md)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)' }}>
          <Checkbox
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange('required', e.target.checked ? true : undefined)}
          />
          <div>
        ...
        {['text', 'email', 'phone', 'textarea'].includes(field.kind) && (
          <>
            <Input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange('minLength', e.target.value ? parseInt(e.target.value) : undefined)}
            ...
            <Input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange('maxLength', e.target.value ? parseInt(e.target.value) : undefined)}
            ...
            <Input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange('pattern', e.target.value ? { value: e.target.value } : undefined)}
              placeholder="^[0-9]+$"
        ...
        {field.kind === 'number' && (
          <>
            <Input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange('min', e.target.value ? parseFloat(e.target.value) : undefined)}
            ...
            <Input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange('max', e.target.value ? parseFloat(e.target.value) : undefined)}
