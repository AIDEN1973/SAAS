...
8-3. 지역 통계 및 벤치마킹 (Phase 2+ 전용)

⚠️ 중요: 이 규칙은 Phase 2 (1~3k 테넌트) 이상에서 적용합니다.

MVP/Phase 1에서는 기본 Analytics(매장별 KPI만)만 구현하며, 지역 벤치마킹과 지도 기반 분포는 Phase 2+에서 도입합니다.

지역 통계 관련 테이블:

- core_regions: 지역 마스터 데이터 (동/구/시/전국 계층)
- core_stores: 매장 정보 (region_id, latitude, longitude 포함)
- analytics.daily_store_metrics: 매장 단위 일별 KPI (Phase 2+)
- analytics.daily_region_metrics: 지역 단위 집계 KPI (Phase 2+)
...
12. 지역 통계 및 지도 기능 규칙 (Phase 2+ 전용)

⚠️ 중요: 이 섹션은 Phase 2 (1~3k 테넌트) 이상에서 적용합니다.
