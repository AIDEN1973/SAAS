...
export interface FormFieldLayoutProps {
  ...
  rowSpan?: number;  // Grid row span (1-12, 기본값 1)
  className?: string;
}
 ...
 *
 * 스키마 필드를 colSpan/rowSpan으로 Grid 레이아웃으로 변환합니다.
...
export const FormFieldLayout: React.FC<FormFieldLayoutProps> = ({
  ...
  // Grid row span 계산 (선택적)
  const gridRowSpan = rowSpan ? `span ${Math.max(1, Math.min(12, rowSpan))}` : undefined;
