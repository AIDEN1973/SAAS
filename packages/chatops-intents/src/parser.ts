 ...
 * 파서 성능 및 보안 제한 상수
 *
 * [SSOT] 이 상수는 infra/supabase/functions/_shared/intent-parser.ts의 PARSER_LIMITS와
 * 동일한 값을 유지해야 합니다. 이 파일이 SSOT입니다.
 *
...
export function parseIntentFromResponse(
  ...
  for (const jsonBlock of blocksToTry) {
    ...
    if (!hasIntent(parsed.intent_key)) {
      errors.push({
        code: ParseErrorCode.INTENT_NOT_FOUND,
        message: `Intent가 Registry에 등록되어 있지 않습니다: ${parsed.intent_key}`,
      });
      continue;
    }

    // Intent Registry에서 Intent 정보 조회
    const intent = getIntent(parsed.intent_key);
    if (!intent) {
      // hasIntent에서 true였는데 getIntent에서 undefined인 경우는 이론적으로 불가능하지만 방어 코드
      errors.push({
        code: ParseErrorCode.INTENT_NOT_FOUND,
        message: `Intent를 조회할 수 없습니다: ${parsed.intent_key}`,
      });
      continue;
    }
    ...
    // params 스키마 검증 (Intent Registry의 paramsSchema 사용)
    try {
      intent.paramsSchema.parse(params);
    } catch (schemaError) {
      errors.push({
        code: ParseErrorCode.SCHEMA_VALIDATION_FAILED,
        message: `params가 스키마 검증에 실패했습니다: ${schemaError instanceof Error ? schemaError.message : String(schemaError)}`,
        details: { params, schemaError },
      });
      continue;
    }

    // 모든 검증 통과 - 성공
    // 타입 안정성: 위의 검증을 통해 automation_level과 execution_class가 유효한 값임이 보장됨
    const parsedIntent: ParsedIntent = {
      intent_key: parsed.intent_key,
      // validateAutomationLevelAndExecutionClass와 automation_level 일치 검증을 통과했으므로 안전
      automation_level: parsed.automation_level as 'L0' | 'L1' | 'L2',
      ...(intent.automation_level === 'L2' && {
        // execution_class 일치 검증을 통과했으므로 안전
        execution_class: parsed.execution_class as 'A' | 'B',
      }),
      // Zod 스키마 검증과 typeof 검증을 통과했으므로 안전
