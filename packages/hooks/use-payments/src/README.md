...
## PaymentFilter.created_at 필터 규칙
...
**SSOT (Single Source of Truth)**: `fetchPayments`는 **항상 배열을 반환**합니다.

- 성공 시: `Promise<Payment[]>` (빈 배열 가능)
- 에러 시: `throw new Error(...)` (예외 발생)
- `response.data`가 없거나 null인 경우: `[]` (빈 배열 반환)

**주의사항**:
- `fetchPayments` 반환값은 항상 배열이므로 `Array.isArray()` 체크는 선택적입니다.
