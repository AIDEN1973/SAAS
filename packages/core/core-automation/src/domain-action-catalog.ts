 ...
 * - packages/core/core-automation/src/domain-action-catalog.ts (이 파일, 최상위 정본)
 * - infra/supabase/functions/_shared/domain-action-catalog.ts (Edge Function 구현)
 *
...
/**
 * Domain Action Catalog 동기화 검증 함수
 *
 * ⚠️ 빌드 타임 검증: 이 함수는 빌드 시 또는 CI에서 호출하여
 * 2개 파일(packages/core/core-automation/src/domain-action-catalog.ts,
 * infra/supabase/functions/_shared/domain-action-catalog.ts)의
 * DOMAIN_ACTION_CATALOG 내용이 일치하는지 검증합니다.
 *
 * @param otherCatalog 다른 파일의 DOMAIN_ACTION_CATALOG 배열
 * @param sourceFileName 검증 중인 파일 이름 (디버깅용)
 * @returns 검증 오류 배열 (오류가 없으면 빈 배열)
 */
export function validateDomainActionCatalogSync(
  otherCatalog: readonly string[],
  sourceFileName: string
): string[] {
  const errors: string[] = [];
  const thisSet = new Set(DOMAIN_ACTION_CATALOG);
  const otherSet = new Set(otherCatalog);

  // 길이 검증
  if (DOMAIN_ACTION_CATALOG.length !== otherCatalog.length) {
    errors.push(
      `[Domain Action Catalog Sync] Length mismatch: main catalog has ${DOMAIN_ACTION_CATALOG.length} items, ` +
      `${sourceFileName} has ${otherCatalog.length} items`
    );
  }

  // 누락된 항목 검증
  for (const item of DOMAIN_ACTION_CATALOG) {
    if (!otherSet.has(item)) {
      errors.push(
        `[Domain Action Catalog Sync] Missing item in ${sourceFileName}: "${item}"`
      );
    }
  }
  ...
  for (const item of otherCatalog) {
    if (!thisSet.has(item as DomainActionKey)) {
      errors.push(
        `[Domain Action Catalog Sync] Extra item in ${sourceFileName}: "${item}"`
      );
    }
  }
