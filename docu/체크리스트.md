...
- [ ] **업종 특화 용어 하드코딩 금지 (Phase 3)**: "학생", "수업", "강사" 등 업종 특화 용어 하드코딩 금지
  - 정본: `useIndustryTerms()` Hook을 통해 동적 용어만 사용
  - 금지: `<PageHeader title="학생 관리" />` → 허용: `<PageHeader title={`${terms.PERSON_LABEL_PRIMARY} 관리`} />`
  - Industry Registry SSOT 참조: `packages/industry/industry-registry.ts`
...
- [ ] **Execution Level 해석 통일**: L0/L1/L2 해석은 정본 기준표(`docu/Industry_Neutrality.md`, `docu/프론트 자동화.md`)에 따라 일관되게 해석
...
8. **Industry Adapter 사용** (업종별 테이블명/FK 관계명 사용 시)
   ...
   - [ ] **적용 범위**: 모든 Edge Functions에서 업종별 테이블명 사용 시 필수
     - L0/L2 핸들러, Task 실행 핸들러, 자동화 Edge Functions 모두 적용
     - ⚠️ **Agent Tools (`agent-tools-final.ts`)**: 15개 Tool 모두 Industry Adapter 적용 확인 필수
   - [ ] **코드 위치**: `infra/supabase/functions/_shared/industry-adapter.ts` (SSOT)
   - [ ] **Agent Tools 검증**: Agent 기반 시스템에서 Tool 구현 시 다음 확인:
     ```typescript
     // ✅ 올바른 예시 (Agent Tool에서 Industry Adapter 사용)
     const tableName = await getTenantTableName(supabase, tenant_id, 'student');
     const { data } = await supabase.from(tableName).select('*').eq('tenant_id', tenant_id);

     // ❌ 잘못된 예시 (하드코딩)
     const { data } = await supabase.from('academy_students').select('*');
     ```
...
2. **`docu/SSOT_UI_DESIGN.md`**: UI 디자인 시스템, Design Tokens, UI Core Component 카탈로그, 컴포넌트별 사용 예시
3. **`packages/shared-catalog.ts`**: 공통 로직(Hook/Feature/Adapter/Component) 카탈로그
