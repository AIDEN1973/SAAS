...
### 8. UI/UX 규칙 확인
- [ ] Tailwind 클래스를 직접 사용하지 않고 Atlaskit 컴포넌트를 사용하는가?
- [ ] Schema Engine을 통해서만 UI를 렌더링하는가? (Atlaskit 컴포넌트 직접 사용 금지)
- [ ] 앱별 `index.css`는 비어있거나 앱 전용 스타일만 포함하는가?
- [ ] `window.alert`, `window.confirm`, `window.prompt`를 사용하지 않고 Atlaskit ModalDialog를 사용하는가?
- [ ] 모바일 환경에서는 모달을 우선 사용하는가? (페이지 전환 최소화)
- [ ] 간단한 수정은 모달, 복잡한 작업은 페이지로 구분하는가?
...
### 19. 파일 생성 규칙 확인 (Critical)
- [ ] *.md 파일을 자동 생성하지 않았는가? (사용자 명시 요청 제외)
- [ ] *.txt 파일을 자동 생성하지 않았는가? (사용자 명시 요청 제외)
- [ ] 기존 문서 수정만 수행했는가?
