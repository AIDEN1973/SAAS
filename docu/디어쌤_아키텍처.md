...
2) capacity_optimization: 정원/시간표/수업 운영 최적화
...
본 문서는 **"마스터 개요 + Cross-Reference 허브"** 역할을 수행하며, 각 도메인별 상세 스펙은 위 전용 문서를 정본(SSOT)으로 따른다.

**⚠️ 문서 정본(SSOT) 규칙:**

각 도메인별로 단일 진실 원천(SSOT) 문서가 지정되어 있으며, 다른 문서는 이를 참조만 합니다:

- **TaskCard/자동화**: 프론트 상황 신호 수집 문서 (정본, StudentTaskCard는 학생용 별칭)
- **ChatOps**: 챗봇.md (정본, ChatOps 기능 제어 시스템 기술명세)
- **Auth/RLS**: 전체 기술문서.txt PART 18 (정본)
- **Regional Analytics**: 디어쌤 통계문서.txt (정본)
- **전체 아키텍처**: 이 문서 (`docu/디어쌤  아키텍처.md`, 정본)

**⚠️ 중요**: 스펙이 여러 문서에 흩어져 있으면 충돌이 발생하므로, 각 도메인별로 정본 문서 1개를 지정하고 나머지는 참조만 합니다.
...
사용자가 어떤 기능을 찾아서 조작하기보다, **서버가** 먼저 판단·추천·**생성**하며, 필요한 경우에만 사용자가 승인·확인하도록 한다.

즉, 학원 원장/선생님은 로그인하지 않아도 운영이 돌아가는 시스템을 경험하게 된다.

**⚠️ Architecture v3.3 실행 주체 분리 (정본 규칙):**

본 문서에서 "시스템이 정책에 따라 실행한다"는 표현은 다음을 의미합니다:
- **AI/Rule Engine**: 판단 및 추천 생성 (서버)
- **Edge Function**: 실제 실행 및 Role 검증 (서버)
- **프론트엔드**: 승인 요청만 (UI는 실행하지 않음)
...
본 문서는 Multi-SaaS Monorepo 중 '학원 업종(Academy)'을 **설명 예시**로 사용하되, 학원은 기본 업종 예시일 뿐 정본이 아님을 명시한다.

**⚠️ 중요: 학원은 예시일 뿐, 정본이 아님**
- 학원(Academy)은 설명을 위한 기본 업종 예시일 뿐이다.
- Core Platform / Event Engine / Payment / Notification / Regional Analytics / AI 구조는 모든 업종(Salon, Nail, Real Estate 등)에서 동일하게 재사용되는 공통 로직이다.
- 업종별 차이는 Industry Adapter 레이어에서만 허용된다.
...
2. 수업/강사 관리(Class & Teacher Management)
...
- **TaskCard**: 업무 카드, **서버가** 생성하는 업무 목록 (StudentTaskCard는 학생용 별칭)
...
#### 1.1.1 Shared Feature 정의 (업종 독립)

**위치:** `apps/*/features/*` (앱 내부) 또는 `packages/schema-engine/*`

**목적:** 여러 페이지에서 재사용되는 사용자 플로우/기능 제공

**정의:**
- 여러 페이지에서 재사용되는 사용자 플로우/기능
- UI + 상태 + 정책 + 로딩/에러 포함 가능
- 데이터 접근은 Hook/Service로 위임

**물리 위치:**
- **UI 비중이 큰 플로우:** `apps/*/features/` (앱 내부 모듈)
  - 예: `apps/academy-admin/src/features/ProtectedRoute.tsx`
  - 예: `apps/academy-admin/src/features/RoleBasedRoute.tsx`
- **도메인/데이터 접근:** 기존대로 `hooks/services/industry/core`로 분리
- **SDUI 엔진:** `packages/schema-engine/*` (Shared Feature 성격)

**불변 규칙:**
- Shared Feature는 기존 의존성 방향 규칙을 준수합니다.
- `apps/* → features/* → hooks/* → services/* → industry/* → core/* → DB` 순서로 의존합니다.
- Shared Feature 내부에서 직접 DB 호출 금지 (Hook/Service를 통해서만 접근).

**참고:** `docu/rules.md` Layer Classification Rules 섹션
...
#### 1.2.1 Domain/Service 전체 목록 (업종 독립)

**위치:** `packages/core/*` (업종 공통) + `packages/industry/*` (업종별)

**목적:** 모든 업종에서 공통으로 사용되는 비즈니스 로직, 도메인 모델, 공통 스키마 제공

**분류:** Domain/Service (비즈니스 로직. 업종별 도메인: `packages/industry/*`. 업종 공통 도메인: `packages/core/*`)

**업종 공통 Domain/Service (`packages/core/*`):**

**현재 구현된 Core 모듈 (20개):**

| 모듈 | 경로 | 용도 | 업종 독립성 |
|------|------|------|------------|
| core-activity | `packages/core/core-activity` | Activity Feed / 타임라인 이벤트 기록 | ✅ |
| core-analytics | `packages/core/core-analytics` | 통계 파이프라인 | ✅ |
| core-auth | `packages/core/core-auth` | 인증 (로그인/회원가입) | ✅ |
| core-automation | `packages/core/core-automation` | 자동화 엔진 (업종 독립) | ✅ |
| core-billing | `packages/core/core-billing` | 과금/청구 | ✅ |
| core-calendar | `packages/core/core-calendar` | 일정/예약/수업 스케줄 | ✅ |
| core-community | `packages/core/core-community` | 게시판/댓글/공지 | ✅ |
| core-config | `packages/core/core-config` | 설정 관리 | ✅ |
| core-consultation | `packages/core/core-consultation` | 상담/기록 관리 | ✅ |
| core-coupons | `packages/core/core-coupons` | 쿠폰/할인 관리 | ✅ |
| core-events | `packages/core/core-events` | 이벤트/프로모션 관리 | ✅ |
| core-metering | `packages/core/core-metering` | 사용량 측정 | ✅ |
| core-notification | `packages/core/core-notification` | 알림 시스템 | ✅ |
| core-party | `packages/core/core-party` | 회원/고객 공통 모델 | ✅ |
| core-payment | `packages/core/core-payment` | 결제 시스템 | ✅ |
| core-reviews | `packages/core/core-reviews` | 리뷰/평가 시스템 | ✅ |
| core-schema-registry | `packages/core/core-schema-registry` | 스키마 레지스트리 | ✅ |
| core-search | `packages/core/core-search` | Full Text Search | ✅ |
| core-storage | `packages/core/core-storage` | 파일 업로드/권한 | ✅ |
| core-tags | `packages/core/core-tags` | 공통 태깅 시스템 | ✅ |
| core-tenancy | `packages/core/core-tenancy` | 테넌시 관리 | ✅ |
| core-tenancy-referral | `packages/core/core-tenancy-referral` | B2B 추천인 코드 | ✅ |
| pii-utils | `packages/core/pii-utils` | PII 유틸리티 | ✅ |

**공통 모듈 구조 패턴:**
```
packages/core/{module-name}/
├── src/
│   ├── index.ts      # 타입만 export (서버 코드는 /service 경로에서만 import)
│   ├── service.ts    # 서버 사이드 서비스 로직
│   └── types.ts      # 타입 정의
├── package.json
└── tsconfig.json
```
...
**불변 규칙:**
- Core Layer 모듈의 `index.ts`는 타입만 export하며, 서버 코드는 `/service` 경로에서만 import합니다.
- Core Layer 모듈은 Industry Layer를 import하지 않습니다 (단방향 의존성).
- Core Layer 모듈은 다른 Core Layer 모듈을 import할 수 있습니다.
- 모든 Core 모듈은 업종 독립적으로 설계되어 모든 업종에서 재사용 가능합니다.

**업종별 Domain/Service (`packages/industry/*`):**

| 업종 | 경로 | 용도 | 업종 독립성 |
|------|------|------|------------|
| Academy | `packages/industry/industry-academy` | 학원 업종별 비즈니스 로직 | ⚠️ Academy 전용 |
| Salon | `packages/industry/industry-salon` | 미용실 업종별 비즈니스 로직 | ⚠️ Salon 전용 |
| Real Estate | `packages/industry/industry-real-estate` | 부동산 업종별 비즈니스 로직 | ⚠️ Real Estate 전용 |
| Gym | `packages/industry/industry-gym` | 체육관 업종별 비즈니스 로직 (향후 추가) | ⚠️ Gym 전용 |

**참고:** Automation & AI Industry-Neutral Rule (본 문서 55-100줄)에 따라 자동화/AI의 실행 구조는 Core Platform 공통 로직입니다.

#### 1.2.2 Service/UseCase 공통화 패턴 (업종 독립)

**위치:** `packages/services/*` + `@industry/*/service`

**목적:** Industry Layer를 래핑하여 제공하는 공통 서비스 레이어

**분류:** Service/UseCase (도메인 로직/데이터 접근 캡슐화. UI/Router 의존 금지. React 컴포넌트에서 직접 DB 호출 금지 원칙의 실행 주체)

**현재 구현된 Service 패키지 (4개):**

| Service 패키지 | 경로 | Industry Layer 매핑 | 용도 | 업종 독립성 |
|---------------|------|-------------------|------|------------|
| attendance-service | `packages/services/attendance-service` | `@industry/academy` | 출석 관리 | ⚠️ Academy 전용 (향후 업종별 Service 추가) |
| auth-service | `packages/services/auth-service` | `@core/auth`, `@core/tenancy` | 인증/회원가입 | ✅ |
| class-service | `packages/services/class-service` | `@industry/academy` | 수업 관리 | ⚠️ Academy 전용 (향후 업종별 Service 추가) |
| student-service | `packages/services/student-service` | `@industry/academy` | 학생 관리 | ⚠️ Academy 전용 (향후 업종별 Service 추가) |

**Service Layer 구조 패턴:**
```
packages/services/{service-name}/
├── src/
│   ├── index.ts      # 타입만 export (클라이언트에도 사용 가능)
│   ├── service.ts    # 서버 사이드 서비스 로직 (Industry Layer 래핑)
│   └── types.ts      # 타입 정의
├── package.json
└── tsconfig.json
```
...
- Service Layer의 `index.ts`는 타입만 export하며, 서버 코드는 `/service` 경로에서만 import합니다.
- 클라이언트는 `@services/{service-name}`를 통해 타입을 import합니다.
- 서버는 `@services/{service-name}/service`를 사용하거나 `@industry/{업종}/service`를 직접 사용합니다.
- Service Layer는 Zero-Trust 아키텍처의 핵심 구성 요소입니다.

**의존성 방향:**
```
Service Layer → Industry Layer → Core Layer → DB
```

**참고:** `docu/rules.md` 105줄, `docu/전체 기술문서.txt` 3953-3963줄

#### 1.2.3 lib 공통 유틸리티 전체 목록 (업종 독립)

**위치:** `packages/lib/*`

**목적:** 모든 업종에서 공통으로 사용되는 유틸리티 제공
...
**참고:** `docu/rules.md` 5. 시간 & 타임존(KST) 규칙, `docu/전체 기술문서.txt` 19-1. 타임존(KST) 표준

#### 1.2.4 Cross-cutting Concern 정립 (업종 독립)

**위치:** `packages/lib/*` + `packages/api-sdk/*` + `packages/env-registry/*` + `packages/core/core-config/*` + `packages/ui-core/src/components/ErrorBoundary.tsx`

**목적:** 로깅/정책/권한/추적/에러처리/PII 등 전 영역에 적용되는 공통 규칙과 유틸 제공

**분류:** Cross-cutting Concern (로깅/정책/권한/추적/에러처리/PII 등 전 영역에 적용되는 공통 규칙과 유틸)

**현재 구현된 Cross-cutting Concern 패키지:**

| 패키지 | 경로 | 주요 기능 | 용도 | 업종 독립성 |
|--------|------|----------|------|------------|
| lib/date-utils | `packages/lib/date-utils` | KST 타임존 헬퍼 | 날짜/시간 변환 | ✅ |
| lib/supabase-client | `packages/lib/supabase-client` | Supabase 클라이언트 유틸리티 | DB 접근 추상화 | ✅ |
| api-sdk | `packages/api-sdk` | Zero-Trust API SDK | API 요청 추상화, 권한 주입 | ✅ |
| env-registry | `packages/env-registry` | 환경변수 레지스트리 | 환경변수 접근 통제 | ✅ |
| core-config | `packages/core/core-config` | 설정 관리 | 설정 접근 통제 | ✅ |
| ErrorBoundary | `packages/ui-core/src/components/ErrorBoundary.tsx` | 에러 경계 | 에러 처리 | ✅ |
| pii-utils | `packages/core/pii-utils` | PII 유틸리티 | 개인정보 마스킹 | ✅ |

**불변 규칙:**
- Cross-cutting Concern은 모든 레이어에서 사용 가능합니다.
- 로깅/추적은 전 영역에 적용되며, 각 레이어에서 일관된 방식으로 사용합니다.
- 정책/권한은 Zero-Trust 원칙에 따라 서버/Edge Function에서만 결정합니다.
- 에러처리는 ErrorBoundary와 api-sdk Error Handling을 통해 일관되게 처리합니다.

**참고:** `docu/rules.md` Layer Classification Rules 섹션

#### 1.2.5 design-system Theme Engine 상세 (업종 독립)

**위치:** `packages/design-system`

**목적:** 업종 독립적인 Multi-Tenant Theme Engine 제공

**Theme Merge Priority:**
1. system default tokens (시스템 기본 토큰)
2. industry tokens (업종별 토큰) - `industry_themes` 테이블에서 조회
3. tenant theme override (테넌트별 테마 오버라이드) - `tenant_theme_overrides` 테이블에서 조회
4. dark mode (다크모드)
...
const theme = createTheme({
  ...
  industryTheme: { tokens: { ... } }, // 업종별 토큰
  tenantTheme: { tokens: { ... } }, // 테넌트별 테마 오버라이드
  highContrast: false,
});
...
- 업종별 차이는 Industry Theme을 통해 처리합니다.
- 테넌트별 커스터마이징은 Tenant Theme Override를 통해 처리합니다.
- Theme Engine은 업종 독립적으로 설계되어 모든 업종에서 재사용 가능합니다.

**참고:** `docu/전체 유아이문서.txt` 512-539줄

#### 1.2.6 api-sdk Zero-Trust API SDK 상세 (업종 독립)

**위치:** `packages/api-sdk`

**목적:** Zero-Trust 원칙을 준수하는 API SDK 제공
...
**ApiClient 클래스 메서드:**
- `get<T>(table, options?)`: GET 요청
- `post<T>(table, data)`: POST 요청
- `put<T>(table, id, data)`: PUT 요청
- `delete<T>(table, id)`: DELETE 요청
- `call<T>(endpoint, method, body?)`: 커스텀 엔드포인트 호출
- `invokeFunction<T>(functionName, body?)`: Edge Function 호출

**Zero-Trust 원칙:**
- UI에서 `fetch`, `axios`, `supabase client`를 직접 호출하지 않습니다.
- 모든 요청은 SDK를 통해서만 수행합니다.
- SDK가 자동으로 `tenant_id`, `industry_type`, `auth token`을 삽입합니다.
- UI는 "무슨 테넌트인지" 또는 "어떤 업종인지" 이해하지 않아도 됩니다.
- UI가 접근할 수 있는 데이터 범위는 RLS가 책임집니다.

**Error Handling 규칙:**
- 인증 만료: JWT 토큰 만료 시 자동으로 로그인 페이지로 리다이렉트
- 테넌트 변경: Context의 `tenantId` 변경 시 모든 활성 쿼리 자동 무효화
- Industry Mismatch: 요청한 `industry_type`과 실제 데이터의 `industry_type` 불일치 시 오류 반환
- 네트워크 오류: 일시적 네트워크 오류는 자동 재시도 (최대 3회)

**Edge Function 호출 규칙:**
- Edge Function 호출도 반드시 `@api-sdk/core`를 통해서만 수행해야 합니다.
- UI는 절대 직접 `fetch`로 Edge Function을 호출하지 않음
- `apiClient.invokeFunction()`을 사용하여 Edge Function 호출
- `apiClient`는 자동으로 tenant_id, industry_type, JWT 토큰을 포함하여 요청

**불변 규칙:**
- 모든 API 요청은 `@api-sdk/core`를 통해서만 수행합니다.
- SDK는 업종 독립적으로 설계되어 모든 업종에서 재사용 가능합니다.

**참고:** `docu/전체 유아이문서.txt` 359-410줄, `docu/스키마엔진.txt` 80-89줄
...
⚠️ 중요: 플랫폼 권한(super_admin/developer)은 DB 기반(user_platform_roles)을 단일 진실 원천(SSOT)으로 사용합니다.
JWT role claim은 표시/UX 힌트 수준으로만 사용하거나 완전 제거합니다.
...
⚠️ 중요: 플랫폼 권한(super_admin/developer)은 DB 기반(user_platform_roles)을 단일 진실 원천(SSOT)으로 사용합니다.
JWT role claim(`auth.jwt()->>'role'`)은 보안 판단에 사용하지 않으며, 표시/UX 힌트 수준으로만 사용하거나 완전 제거합니다.
...
- AI가 제공한 추천값을 기반으로 서버가 실행한 모든 의사결정은 다음을 로그로 보관:
  - 시간
  - 입력 데이터
  - 판단 근거
  - 실행 결과
...
- 04:00 서버가 청구 생성
- 월말 서버가 리포트 생성 (AI 호출 포함)
- 매일 아침 서버가 AI 데일리 브리핑 생성 (07:00, AI 호출 포함)

**Behavior-based Events (사용자 행동 기반)**
- 학생 등원/하원 이벤트 발생 시 서버가 출결 메시지 발송
...
| 수업 생성 Form | 100% SDUI | 없음 | Form Schema로 완전 자동 생성 |
...
사용자가 무엇을 해야 할지 고민하기 전에 서버가 먼저 분석·제안·요약한다 (AI 호출 포함).
...
**Level 2 — TaskCard (task_type: 'ai_suggested', entity_type='student') (추천 + 초안 제공)** (StudentTaskCard는 학생용 별칭)

**⚠️ v3.3 정본: "AI Suggestion"은 독립 개념이 아닙니다.**
- TaskCard (task_type: 'ai_suggested', entity_type='student')로 통합됨 (StudentTaskCard는 학생용 별칭)
- 서버/Edge Function에서 생성
- 프론트엔드는 승인 요청만 처리
- 행동은 사용자가 결정
- 예:
  - "안부 문자 초안 준비됨 → 발송하시겠습니까?"
  - "이번 수업 출석률 저조 → 원인을 분석할까요?"

**Level 3 — AI Auto-Action (서버가 정책 해석 후 실행)**

**⚠️ 실행 주체 명시:**
- AI/Rule Engine: 판단 (서버)
- Edge Function: 실행 (서버)
- 프론트엔드: 승인 요청만 (UI는 실행하지 않음)
- 사용자 개입 없이 시스템 자체 실행
- 예:
...
**예:**
- 수업 시작 10분 전 → 출석부 바로가기 배너 표시 (클릭 시 진입)
- 수업 종료 후 → 상담일지 작성 화면을 자동 추천
- 월말 → 청구 요약 카드 우선순위 가중치 증가 (강조 표시, 그룹 순서는 불변)
- 비가 많이 오는 날 → **서버가** 하원 안전 안내문 TaskCard **생성** (StudentTaskCard는 학생용 별칭)
...
| 실장(Sub Admin) | 학생/수업/출결/수납 일부 접근 |
...
| 서버가 상담 AI 요약 생성 | 실행 | 실행 | 요청만 (실행 X) | X | X |
...
⚠️ **AI 토글 정책 우선순위 규칙:**
- AI 관련 메뉴/섹션은 역할별 숨김 규칙과 무관하게 항상 표시됩니다 (숨김 금지).
- 역할별 UI 숨김 규칙은 AI 토글 정책보다 우선순위가 낮습니다.
- AI 기능이 비활성화(`effective_ai_enabled=false`)되어도 UI는 숨기지 않고, 실행만 차단합니다.
...
- 실제 시스템 role 값(enum/문자열): 소문자 snake_case 사용 (`admin`, `sub_admin`, `instructor`, `teacher`, `assistant`, `guardian`, `parent`)  -- instructor/guardian은 정본 키, teacher/parent는 backward compatibility
...
- 예: academy는 "수업 관리" 메뉴, salon은 "예약 관리" 메뉴
...
- 청구서 생성 시 즉시 알림 발송
- 알림 클릭 시 결제 화면으로 이동 (사용자 클릭 필요)

**결제 성공 시 영수증 자동 발송**
- 결제 완료 즉시 서버가 영수증 생성 및 발송
- SMS/앱 내 알림으로 제공
...
#### 3.1.1 학생 관리 홈(Home) — 학생 관리 대시보드

**라우팅 및 화면 구조**

학생 메뉴 첫 진입 시 라우트는 `/students/home`이며, 기본 화면은 학생 관리 전용 대시보드이다.

전체 학생 목록은 `/students/list`로 분리되며, 홈 화면에서는 별도의 "전체 학생 보기" 버튼으로만 진입 가능하다.

**⚠️ 참고: Student Task Cards는 `/home` 대시보드에서 표시됩니다.**
- `/students/home`은 학생 관리 전용 통계/액션/활동 대시보드
- `/home`은 전체 업무 대시보드 (Student Task Cards 포함)

**학생 관리 대시보드 구성**

**1. 학생 통계 카드 섹션**
- **전체 학생 수**: 전체/활성/비활성/신규 등록 학생 수
- **오늘 출석률**: 출석/지각/결석/미체크 현황
- **오늘 지각률**: 오늘 지각한 학생 비율
- **오늘 결석률**: 오늘 결석한 학생 비율
- **활성 학생 수**: status='active'인 학생 수
- **비활성 학생 수**: status='inactive'인 학생 수
- **수업당 평균 인원**: 전체 학생 수 / 활성 수업 수
- **평균 정원률**: (현재 인원 / 정원) × 100
- **ARPU (학생 1인당 매출)**: 이번 달 매출 / 전체 학생 수
- **이번 주 평균 출석률**: 최근 7일 출석률 평균
- **학생 알림**: 위험 학생/결석 학생/상담 대기 건수
- **상담 통계**: 이번 달 상담 건수/대기 중/긴급 상담 건수
...
각 카드를 클릭하면 해당 기능 화면으로 이동하며, 학생 관리 모듈의 진입점 역할을 한다.
...
TaskCard {
  ...
  entity_id: UUID      // 업종 중립 엔티티 ID (정본)
  entity_type: string  // 업종 중립 엔티티 타입 (정본, 예: 'student', 'client', 'customer')
  student_id: UUID     // 레거시 필드 (하위 호환성, entity_type='student'인 경우 entity_id와 동일)
  source?: 'attendance' | 'billing' | 'behavior' | 'weather' | 'proactive_analysis' // TaskCard (task_type: 'ai_suggested') 근거
  priority: number (0-100) // 정본: 0-100 스케일
  title: string
  description: string
  action_url: string
  suggested_action?: jsonb // 액션 정의 (메시지 초안, 분석 요청 등)
  dedup_key?: string // 중복 방지 키: "{tenantId}:{trigger}:{entityType}:{entityId}:{window}"
  status?: 'pending' | 'approved' | 'executed' | 'expired' // 승인 상태
...
카드의 `action_url`은 `task_type`에 따라 아래 규칙으로 서버가 생성한다:
...
**신규 등록 학생 환영 카드 (new_signup) 타이틀/설명 형식:**
- 타이틀: "{학생이름} 학생 신규 등록" (예: "박소영 학생 신규 등록")
- 설명: "환영 메시지를 발송해 보세요."

**URL 생성 규칙:**
- `entity_id`는 카드의 `entity_id` 필드 값으로 치환 (정본)
- 레거시: `student_id`는 카드의 `student_id` 필드 값으로 치환 (하위 호환, entity_type='student'인 경우 entity_id와 동일)
- 카드 클릭 시 해당 URL로 라우팅
...
**우선순위 계산:**
- 긴급도 (1-40) + 시간대 (1-30) + 담당 수업/학생 (1-30)
- 최종 priority = 긴급도 + 시간대 + 담당 수업/학생

#### 3.1.3 학생업무카드(TaskCard) 생성 규칙 (StudentTaskCard는 학생용 별칭)
...
- 이탈 위험 감지: 서버가 이탈 위험 감지 시 즉시 카드 생성 (AI 호출 포함)
- 신규 등록: 학생 등록 시 환영 카드 생성

**⚠️ M1: TaskCard 만료 규칙(expires_at) 통일 (v3.3 확정, StudentTaskCard는 학생용 별칭):**
...
**⚠️ 다른 섹션에서 TaskCard 만료 규칙을 언급할 때 (StudentTaskCard는 학생용 별칭):**
...
```typescript
task_card_dedup = {
  // 동일 task_type + 동일 entity_type + 동일 entity_id + 동일 날짜 → 1장만 생성
...
정렬 기준: 긴급도 → 시간대 → 담당 수업/학생

#### 3.1.4 전체 학생 목록(List View) — 검색 및 상세

**역할 및 진입 경로**

이 화면은 업무 홈이 아닌 "고급/상세 조회 기능"이며, 기본 진입 동선은 학생 관리 홈(`/students/home`)에서 "전체 학생 보기" 버튼 또는 상단 탭 전환을 통해서만 가능하다.

**라우팅:** `/students/list`

**학생 기본 정보 조회**
- 이름, 학부모, 연락처, 대표수업만 표시
- 학생 검색 및 기본 필터(학년, 수업, 상태)
...
**⚠️ 변경 이력 (2026-01-14): PII 마스킹 완전 제거**

**제거 이유:**
1. **관리자 운영 효율성 저하**: 마스킹으로 인해 AI 요약 및 추천 내용이 두리뭉실하고 실행 불가능한 정보 제공
   - 예: "이탈 위험 학생 조기 발견: 3명이 감지됨" → 실제 학생명이 없어 즉시 조치 불가
   - 예: 뉴스 기사 내용이 `[개인] [개인] [개인]`으로 마스킹되어 LLM이 원문과 무관한 요약 생성

2. **업무 흐름 단절**: 관리자가 추천사항을 보고 다시 원본 데이터를 찾아야 하는 이중 작업 발생

3. **보안 맥락**: 모든 AI 처리는 테넌트 격리된 내부 시스템에서만 발생하며, 외부 유출 경로 없음
   - Zero-Trust 원칙: tenant_id는 JWT에서만 추출
   - RLS 정책으로 데이터 접근 제어
   - OpenAI API는 데이터를 학습에 사용하지 않음 (Enterprise 정책)

**제거 범위:**
- `consultation-ai-summary`: 상담일지 AI 요약 시 PII 마스킹 제거 (2026-01-14)
- `proactive-recommendation`: 선제적 추천 시 학생명 포함으로 변경 (2026-01-14)
- `ai-briefing-generation`: 일일 브리핑에 학생명 포함으로 변경 (2026-01-14)
- `generate-performance-insights`: LLM 프롬프트를 구체적 정보 제공으로 개선 (2026-01-14)

**구현 상태:**
- ✅ 모든 Edge Functions에서 마스킹 로직 제거 완료
- ✅ LLM 프롬프트를 "구체적이고 자세한 정보 제공" 방향으로 개선
- ✅ 업종 중립적 용어로 변경 (학생→구성원, 강사→담당자, 수업→그룹 등)
- ✅ Edge Functions 재배포 완료 (2026-01-14 최종)
- ✅ 문서 업데이트 완료 (2026-01-14)

**업종 중립성 준수 (Industry Neutrality):**
- ✅ AI 프롬프트에서 "학원", "학생", "강사" 등 업종 특정 용어 제거
- ✅ "운영 데이터 분석 전문가", "구성원", "담당자" 등 중립적 용어 사용
- ✅ Automation & AI Industry-Neutral Rule (SSOT) 준수
- ✅ 향후 미용실, 네일샵 등 다양한 업종 확장 가능

**향후 방침:**
- 관리자용 AI 기능에는 PII 마스킹을 적용하지 않음
- 만약 향후 학부모용 AI 기능이 추가될 경우, 해당 기능에 한해 선택적 마스킹 검토
- 현재 설계상 학부모는 자녀 본인의 정보만 접근 가능하므로 마스킹 불필요
- AI 기능은 업종 중립적으로 유지하여 확장성 보장
...
- 한 학생이 여러 수업에 동시에 등록 가능
- "대표 수업(primary class)" 개념을 도입하여 리스트 기본 표시에 사용
- 출결/청구/통계에서 class_id 기준 집계를 안전하게 처리

**UI 요구사항:**

- 학생관리 기본 화면에는 최소 정보(이름·학부모·연락처·대표수업)만 표시하며,
- 태그·일괄등록·다중수업 관리 등 고급 기능은 '고급 옵션(Advanced)' 버튼 클릭 시 나타난다.

### 3.2 수업/강사(Class & Teacher) 도메인

#### 3.2.1 Today-First 기준

**신규 기본 화면 = "오늘 수업 있는 수업"**

현재 시간 기준으로 곧 시작할 수업이 상단 고정
- 수업별 총원/출석/지각 간단 표시
- 서버가 추천 생성 (AI 호출 후 서버가 정책 해석하여 강조):
  - "출석률 저조 수업"
  - "강사 일정 충돌 가능성 있음"
...
**Core Platform (업종 공통):**
- 수업 기본 정보 (이름, 시간표, 강사 배정)
- 학생-수업 다중 소속 관리
- 시간표 충돌 감지 (기본 로직)

**Industry Layer (업종별 차별화):**
- 수업 자동 색상 태깅 (학원: 과목별, 미용실: 서비스별)
- 부담임 설정 (학원 전용)
- 수업별 지각 기준 설정 (학원 전용)
- 수업별 수업료 설정 (학원 전용)
...
| 수업 자동 색상 태깅 | industry-academy |
| 부담임 설정 | industry-academy |
| 수업별 지각 기준 (class_specific_late_rules) | industry-academy |
| 수업별 수업료 설정 | industry-academy |

**⚠️ 모든 Class 기능에 대해 "Core vs Industry" 구분 주석이 필요합니다.**

#### 3.2.2 Schedule Conflict Detection (시간표 충돌 감지)

출석부 바로가기 배너 표시 전에 아래 검증을 수행한다. (정본: 자동 열림 금지, 클릭 시 진입)
...
**충돌 시 UI 처리:**
- "시간표 충돌 감지됨" 알림 표시
- "어떤 수업을 우선할지 선택하십시오" 선택 화면 제공
- "AI 추천 수업: 출석률 저조 수업" 추천 정보 제공
- 충돌 해결 전까지 출석부 바로가기 배너 표시 방지 (정본: 자동 열림 금지)

#### 3.2.3 기본 기능

**오늘 수업 있는 수업 목록**
- 오늘 수업이 있는 수업만 기본적으로 표시
- 수업별 학생 수 및 출결 현황 간단 표시
- 수업 시작 10분 전 → 홈/출결 탭에 출석부 열기 카드/배너 표시, 클릭 시 이동 (정본: 자동 화면 전환 금지)

**⚠️ M5: 수업 생성 최소 입력 조건 통일 (v3.3 확정):**
...
**수업 편성표(Calendar-like)**
- 캘린더 뷰로 수업 일정 확인
- 요일/시간별 수업 편성표

**수업 자동 색상 태깅**
- 수업별 색상 자동 할당 및 커스터마이징
...
**⚠️ 수업/강사 도메인 Industry Layer 분리:**

"부담임 설정", "수업 자동 색상 태깅"은 Industry Layer에 위치해야 함:
- Core Platform의 "class entity 확장"으로 OR
- `industry-academy` 레이어에 위치
- 현재 문서에는 학생·수업 도메인의 일부로 보이지만, 실제 구현 시 Industry Layer로 분리 필요
...
- 수업 생성 시 최소 입력(수업 이름만)으로도 생성 가능하며,
- 시간·요일·정원·색상 등은 고급 옵션에서 수정한다.
- 수업 목록은 기본적으로 "오늘 수업 있는 수업"만 표시하며,
- 전체 수업 관리 및 편성표는 Advanced 영역으로 이동한다.
...
**1. 키오스크 출석 체크 (Kiosk Check-In)**
- 학생이 태블릿 단말기에서 본인 휴대폰 번호 입력으로 출석 체크
- 인증 불필요 (공개 키오스크 모드)
- 출석 완료 시 보호자에게 자동 알림 발송
- 상세 내용: 3.3.5.1 키오스크 출석 체크 참조

**2. QR 감지 시 자동 등원 처리**
- QR 스캔 시 자동 등원 처리 (사용자 확인 최소화)
- 상세 내용: 3.3.5 QR 출결 아키텍처 참조

**미구현 기능 (선택적):**

**AI 출석 예측 자동 적용 (Phase 3 - 제외)**
- ~~과거 출결 패턴 기반으로 오늘 출석 예상 학생 추천~~
- ~~선생님이 확인/수정만 하면 됨~~
- **제외 이유:** 키오스크 출석 체크로 충분하며, 복잡도 대비 효과가 낮음
- **재검토 조건:** 키오스크 사용률이 50% 미만이고 Teacher 부담이 큰 경우

**검색 없이 "오늘 수업 학생만" 기본 노출**
- 수업별 오늘 수업 학생만 자동 필터링
- 불필요한 검색 과정 제거
...
(출결 설정 JSON 구조는 3.3.7 '출결 설정'의 `attendance.*` 경로 정의를 따른다. 저장 위치는 tenant_settings(key='config').value(JSONB)입니다.)

**핵심 합의: QR은 "학원 고정 QR 코드 + 학생 휴대폰 촬영 방식"**

**QR 코드 방식**

각 학원(및 선택적으로 수업/강의실)별로 고정 QR 코드를 발급한다.
...
**⚠️ B6: 출결 Anti-Abuse 실제 DB 필드 정의:**
...
CREATE TABLE public.attendance_logs (
  ...
  -- ⚠️ B6: Anti-Abuse 필드 (GPS/Auth Method)
  gps_lat numeric(10, 7),    -- GPS 위도 (QR 출결 시 선택적 수집)
  gps_lng numeric(10, 7),    -- GPS 경도 (QR 출결 시 선택적 수집)
  auth_method text,          -- 인증 방법: 'qr', 'manual', 'sms_auth', 'temp_token'
...
**인증 토큰 구분:**
- **Parent Token**: 학부모 앱에서 발급된 JWT (role='guardian' 또는 'parent')  -- guardian은 정본 키, parent는 backward compatibility
...
#### 3.3.5.1 키오스크 출석 체크 (Kiosk Check-In)
...
**구현 배경:**
- 학생이 학원 입구의 태블릿에서 본인 휴대폰 번호만 입력하면 즉시 출석 처리
- QR 코드나 별도 앱 없이 간단한 UX로 출석 체크 가능
- 보호자에게 자동으로 출석 알림 발송 (Zero-Management)

**아키텍처:**

1. **프론트엔드**: `/kiosk-check-in` 페이지
   - 큰 글씨, 간단한 UI (태블릿 전용)
   - 휴대폰 번호 입력 필드 + 출석하기 버튼
   - 성공 시 학생 이름과 출석 완료 시각 표시 (3초 후 자동 숨김)
   - 인증 불필요 (공개 키오스크 모드)

2. **백엔드**: `kiosk-check-in` Edge Function
   - **인증 방식**: 공개 API (인증 불필요, IP/Rate Limiting으로 보안)
   - **Service Role**: RLS 우회하여 학생 조회 및 출석 기록 생성
   - **Policy 제어**: `kiosk.enabled` 설정으로 기능 활성화 제어

3. **데이터베이스**:
   - `students.student_phone`: 학생 본인 휴대폰 번호 (테넌트 내 유니크)
   - `attendance_logs.check_in_method`: 'kiosk_phone' (키오스크 출석 체크 표시)

**키오스크 출석 체크 흐름:**

```
1. 학생이 태블릿에서 휴대폰 번호 입력
   └─ 예: 010-1234-5678 (하이픈 자동 추가)

2. kiosk-check-in Edge Function 호출
   ├─ Policy 확인: kiosk.enabled = true인지 검증
   ├─ 학생 조회: students 테이블에서 student_phone으로 조회
   ├─ 출석 기록 생성: attendance_logs에 check_in_method='kiosk_phone' 기록
   └─ 보호자 알림 발송 (Policy 기반)

3. 보호자 자동 알림 (Zero-Management)
   ├─ Policy 확인: kiosk.auto_notify_guardian = true인지 검증
   ├─ 보호자 정보 조회: guardians 테이블에서 is_primary=true인 보호자 조회
   ├─ 알림 채널: kiosk.notification_channel (기본: 'kakao_at')
   ├─ 알림 템플릿: kiosk.notification_template
   │   └─ 예: "${student_name} 학생이 ${time}에 출석했습니다."
   └─ dispatch-notification Edge Function 호출

4. 성공 응답
   └─ 학생 이름 + 출석 완료 시각 표시 (3초 후 자동 숨김)
```

**Policy 설정 (tenant_settings.config.kiosk):**

```json
{
  "kiosk": {
    "enabled": true,                          // 키오스크 모드 활성화 여부
    "check_in_method": "phone_only",          // 체크인 방식: phone_only (휴대폰 번호만)
    "auto_notify_guardian": true,             // 보호자 자동 알림 발송 여부
    "notification_channel": "kakao_at",       // 알림 채널 (kakao_at | sms)
    "notification_template": "${student_name} 학생이 ${time}에 출석했습니다."  // 알림 템플릿
  }
}
```
...
1. **공개 API**:
   - 인증 불필요 (누구나 접근 가능)
   - IP/Rate Limiting으로 보안 (Supabase Edge Functions 내장 기능)
   - Service Role로 RLS 우회 (학생 조회 및 출석 기록 생성)

2. **학생 본인 휴대폰 번호**:
   - `students.student_phone` 컬럼에 학생 본인 휴대폰 번호 등록 필수
   - 테넌트 내에서 유니크 (중복 불가)
   - 형식: 01012345678 (하이픈 없음, 자동 정규화)

3. **출석 기록**:
   - `attendance_logs.check_in_method`: 'kiosk_phone'로 기록
   - `attendance_logs.status`: 'present'로 고정
   - KST 타임존 적용 (한국 표준시)

4. **보호자 알림**:
   - 보호자 알림 발송 실패는 출석 체크 성공에 영향 없음 (로그만 기록)
   - is_primary=true인 보호자에게만 알림 발송

**데이터베이스 스키마 변경:**

Migration `150_add_kiosk_check_in_support.sql`:

```sql
-- 1. students 테이블에 학생 본인 휴대폰 번호 추가
ALTER TABLE public.students
ADD COLUMN IF NOT EXISTS student_phone TEXT;

-- 2. student_phone 유니크 인덱스 (동일 테넌트 내에서 중복 방지)
CREATE UNIQUE INDEX IF NOT EXISTS idx_students_tenant_phone
ON public.students(tenant_id, student_phone)
WHERE student_phone IS NOT NULL;

-- 3. attendance_logs에 체크인 방법 컬럼 추가
...
- **태블릿 전용**: 큰 글씨, 간단한 UI
- **빠른 출석**: 휴대폰 번호 입력 → 출석하기 버튼 → 완료 (3초 표시)
- **성공 피드백**: 학생 이름 + 출석 완료 시각 표시
- **에러 처리**: 명확한 에러 메시지 표시 (등록되지 않은 번호, 형식 오류 등)
...
- Default Policy: `qr_gps_enabled: false` (테넌트 생성 시 설정값으로 저장)
- 활성화 시 경고 메시지 표시: "모바일 환경에서 GPS 권한 거부 시 자동으로 SMS 인증으로 전환됩니다"

**GPS 설정:**
- GPS 기반 허용 반경 제한 옵션 (`attendance.qr_gps_enabled`, 저장 위치는 tenant_settings(key='config').value(JSONB))
- 기본 허용 반경: 150m (`attendance.qr_gps_radius_meters`, 저장 위치는 tenant_settings(key='config').value(JSONB))
- 학원 위치에서 벗어난 QR 입력 감지
- GPS 검증 실패 시: SMS 인증 또는 본인 확인으로 자동 Fallback
- GPS 권한 거부 시: 자동으로 SMS 인증으로 전환

**2) 비정상 시간대 감지 (Adaptive Rule)**
- 정규 수업 시간이 아닌데 QR 찍힘 감지
- 장시간 외부에서 QR 입력 발생 감지

**3) 동일 학생의 반복 QR 입력 시 경고**
- 짧은 시간 내 동일 학생 QR 반복 입력 감지
- 부정행위 의심 시 본인 인증 요청

**4) AI 기반 "출석 불일치 패턴" 분석**
...
**UI 노출 방식:**
- QR 출결은 Default Policy에 따라 비활성으로 설정되며, 원장이 'QR 출결 사용' 설정을 켜야 QR 기능이 메뉴에 나타난다.
- QR 출결 메뉴는 Default Policy에 따라 숨김으로 설정되며, 설정에서 켜면 노출된다.

#### 3.3.7 출결 설정 (환경설정 > 출결 설정으로 이동)

**⚠️ 출결 설정 통합 원칙:**

출결 설정은 반드시 `attendance.*` 경로를 사용하는 단일 JSON 구조로 통합하여 제어한다. (저장 위치는 tenant_settings(key='config').value(JSONB))
...
**attendance.* 경로 공식 스펙 (단일 소스, 저장 위치: tenant_settings(key='config').value(JSONB)):**
...
```json
{
  "attendance": {
    ...
    // 출결 이상 감지 설정 (자동화 연계)
    "absence_threshold_days": 3,  // 결석 감지 임계값 (일, Default Policy: 테넌트 생성 시 3일로 설정값 저장)
    "anomaly_detection": {
      "enabled": true,  // 출결 이상 감지 활성화 여부 (Default Policy: 테넌트 생성 시 설정값으로 저장)
      "threshold": 3  // 이상 패턴 감지 임계값 (일, Default Policy: 테넌트 생성 시 3일 연속 결석으로 설정값 저장)
    },
...
출결 설정(attendance_settings)은 단일 JSON 구조(`attendance.*` 경로)로 통합되어야 하며, QR 인증/보조 인증/토큰 로직/지각 기준/중복 체크/출결 모드도 모두 이 JSON 구조에서 제어된다. (저장 위치는 tenant_settings(key='config').value(JSONB))
...
**정본 위치:** 3.3.7 출결 설정의 `attendance.*` 경로 공식 스펙 참조 (저장 위치는 tenant_settings(key='config').value(JSONB))

**다른 섹션에서 출결 설정을 언급할 때:**
- "3.3.7 출결 설정의 `attendance.*` 경로 공식 스펙 참조" 형태로만 언급 (저장 위치는 tenant_settings(key='config').value(JSONB))
- 중복된 JSON 구조 정의 제거
...
#### 3.4.1 수납 관리 홈(Home) — 서버가 청구 생성 진행 현황 대시보드
...
**모든 청구서는 서버가 생성 → 관리자가 발송 버튼 누를 필요 없음**
- 월 서버가 청구 생성 (배치)
- 서버가 청구서 발송 (설정 기반)
...
**결제 실패 시 정책 기반 재시도 실행 (Policy 기반)**
- 결제 실패 감지 시 자동 재시도
- 관리자 개입 최소화
...
**이번달 예상 수납률 카드**
- 서버가 예상 수납률 계산 및 표시 (AI 호출 포함)
- 월말 청구 요약 카드 우선순위 가중치 증가 (강조 표시, 그룹 순서는 불변, Adaptive UI)
...
**미납 관리**
- 미납 학생 목록 조회
- AI 추천 타이밍 기반 정책 해석 후 미납 알림 (Policy 기반)
...
closed_invoice_payment_strategy = {
  // 상용화 단계 전략 (Default Policy, 우선순위 1)
  mvp_strategy: {
    // 전략 1: 마감된 invoice를 reopened (상용화 단계 Default Policy)
    reopen_closed_invoice: {
      enabled: true,  // Default Policy: 상용화 단계에서 테넌트 생성 시 enabled=true로 설정값으로 저장
      priority: 1,    // 최우선 전략
  ...
  advanced_strategies: {
    ...
    carry_over_to_next_month: {
      enabled: false,  // Default Policy: 테넌트 생성 시 enabled=false로 설정값으로 저장, Advanced 모드에서만 활성화 가능
  ...
  monthly_closing: {
    ...
    // 마감 후 결제 처리 (상용화 단계: reopen만)
    post_closing_payment: {
      mvp_strategy: 'reopen_closed_invoice',  // 상용화 단계 기본 전략
      advanced_strategies: ['carry_over', 'accounting_conflict'],  // Advanced 모드에서만
      require_admin_approval: false  // 상용화 단계에서는 자동 처리
    }
  }
}
```
...
- 처리 규칙:
  ...
  - 해당 학생의 미납 관련 AI 인사이트 카드 정책 해석 후 삭제 (Policy 기반)
  - 미납 알림 스케줄 취소
  - 관련 이벤트 로그 기록
...
- 조기납부 허용 여부
- Default Policy: 테넌트 생성 시 false로 설정값으로 저장
...
billing_home_metrics_spec = {
  ...
  // 서버가 청구 생성 진행률 = 발송 완료 청구서 수 / 전체 청구서 수
  // 발송 실패 청구서는 계산에서 제외
...
서버가 상황을 감지하고 메시지 초안을 추천 생성 (AI 호출 후 서버가 정책 해석하여 TaskCard 생성, StudentTaskCard는 학생용 별칭)

**예시:**
- "오늘 폭우 지역이라 하원 안내 메시지를 발송하시겠습니까?"
  → 서버가 날씨 정보 기반으로 추천 생성 (AI 호출 후 서버가 정책 해석하여 초안 제공)
- "김○○ 학생 3일 결석 — 학부모 안부 문자 초안이 준비되었습니다."
  → 서버가 결석 패턴 분석 후 추천 생성 (AI 호출 후 서버가 정책 해석하여 초안 생성)
...
**출결 알림**
- 등원/하원 알림 정책 기반 발송 (Policy 설정 기반)
...
### 3.6 지역 기반 통계(Regional Analytics)

(출결버스에 없는 "초격차 기능")

**중요: 기본 지역 통계 기능은 상용화 단계에 포함됩니다.**
...
**location.* 경로 필드 구조 (저장 위치: tenant_settings(key='config').value(JSONB)):**
...
**location.* 경로 확장 구조 (저장 위치: tenant_settings(key='config').value(JSONB)):**
...
- 행정동 코드 → `location_code` (정본)
- 시군구 코드 → `sigungu_code` (정본)
- 시도 코드 → `sido_code` (정본)

**SSOT 매핑 규칙:**
- 다른 통계 문서/테이블에서 `region_code`를 사용하는 경우, `region_code = location_code`로 매핑됩니다.
- 모든 조인/ETL/랭킹엔진에서는 `location_code`를 사용하는 것을 정본으로 합니다.
- `analytics.daily_region_metrics.region_code`는 내부적으로 `location_code`와 동일한 의미입니다.
...
regional_comparison_with_industry_filter = {
  ...
  // tenant_settings KV 구조에서 key='config' row의 value(JSONB) 내부 경로 regional_analytics.comparison_priority로 커스터마이징 가능하나
  // 기본값은 위 순서를 따름
  ...
  minimum_sample_size: {
    ...
    // → tenant_settings KV 구조에서 key='config' row의 value(JSONB) 내부 경로 regional_analytics.minimum_sample_size로 조정 가능하도록 설정
  },
...
minimum_sample_handling = {
  ...
  // 3 <= N < 5이면 경고 메시지와 함께 비교 수행 (Default Policy: 테넌트 생성 시 minimum_sample_size=3으로 설정값 저장되어 있어 이 구간은 사실상 사용 안 됨)
  low_sample: {
...
- 수업별 출결 히트맵은 원천 데이터에서 Staging 테이블로 10분 단위로 집계
- 실시간성보다는 정확성 우선
- 일별 집계 데이터는 매일 자정에 생성

**analytics.daily_region_metrics: 1일 1회 (00:30)**
- 지역 통계는 하루 1회 자정 30분에 집계 (정본 이름: analytics.daily_region_metrics)
- 대량 데이터 처리 최적화
- ⚠️ 참고: regional_metrics_daily는 구버전/폐기된 네이밍입니다. 정본은 analytics.daily_region_metrics를 사용합니다.

**⚠️ 2-4: AI 인사이트 스케줄 (Phase 구분):**

**ai_insight:**
- **상용화 단계**: 주간 브리핑 (매주 월요일 07:00), 주간 모델 재학습, Daily briefing → 07:00 서버가 생성 (AI 호출 포함), 고급 인사이트 실시간 감지
- Risk detection → 실시간 감지 및 업데이트 (상용화 단계부터 적용)
...
- class_attendance_heatmap: 수업별 출결 히트맵
...
- ai_insights: AI 인사이트 논리 테이블 (정본, SSOT, 필요 시 MV로 파생 가능)

**⚠️ 중요: Analytics 테이블 명칭 정본화 (SSOT):**

본 문서에서 기술하는 지역 통계 테이블은 통계문서(RA1)와 기술문서의 정식 테이블명을 기준으로 정본화합니다.
...
**구버전/폐기된 네이밍 (사용 금지):**
- ~~regional_metrics_daily~~ → analytics.daily_region_metrics로 교체
- ~~academy_metrics_daily~~ → analytics.daily_store_metrics로 교체
- ~~analytics.daily_metrics~~ → analytics.daily_store_metrics로 교체
- ~~analytics.monthly_revenue~~ → analytics.daily_store_metrics에서 파생 (필요 시)

**Materialized View/뷰 (성능 최적화용, 선택):**
- 성능이 필요한 경우 `analytics.daily_region_metrics`를 기반으로 MV를 생성할 수 있으나, 정본 테이블은 `analytics.daily_region_metrics`입니다.
- 통계문서(RA1)와 기술문서가 단일 정본(Single Source of Truth)입니다.
...
- 출결 이벤트, 결제 이벤트 등 실시간 발생 → `analytics.events` 테이블에 기록
  - ⚠️ 참고: `analytics.events.event_type`은 자동화 카탈로그의 `event_type`과 다른 도메인 값입니다. `analytics.events.event_type`은 시스템 이벤트 타입(예: `attendance.check_in`, `payment_webhook`)이며, 자동화 카탈로그의 `event_type`은 자동화 시나리오 키(예: `overdue_outstanding_over_limit`, `payment_due_reminder`)입니다.
- Edge Function이 실시간으로 이벤트를 수집하고 전처리

**배치 집계 → Analytics:**
- `analytics.events` → `analytics.daily_store_metrics` (매장 단위 일별 집계, 정본)
- `analytics.events` → `analytics.daily_region_metrics` (지역 단위 집계, 정본)
...
**AI 엔진 → AI 산출물:**
- `analytics.daily_store_metrics` + `analytics.daily_region_metrics` → AI 엔진 입력
- AI 엔진 → `ai_student_risk_scores` (또는 유사 테이블) 생성
- Risk Score 계산 및 이상 탐지 수행

**UI → 사용자 화면:**
- `ai_student_risk_scores` → `TaskCard` 생성 (StudentTaskCard는 학생용 별칭)
- `ai_student_risk_scores` → Emergency 리스트 화면 표시
- `analytics.daily_store_metrics` → Regional Analytics 대시보드 표시

**파이프라인 실행 위치:**
- Edge Function: 실시간 집계 (1분/10분 단위)
- Cron Job: 일일 집계 (analytics.daily_store_metrics, analytics.daily_region_metrics)
...
- 권역별 평균/순위 계산
- 권역 코드는 `location.region_code`에 저장 (저장 위치는 tenant_settings(key='config').value(JSONB))
...
### 3.7 AI 인사이트(AI Insights)

**중요: 기본 AI 분석 기능은 상용화 단계에 포함됩니다.**
...
서버가 AI Risk Score Weight를 적용할 때 업종별(`industry_type`)로 Default Policy가 다르며, Tenant Override가 우선 적용된다.
...
각 카드 클릭 시 상세 분석 화면으로 이동 (사용자 클릭 필요)

**주요 기능**

- 서버가 상담일지 AI 요약 생성
  - 상담일지 작성 시 서버가 AI 요약 생성
- 학생 출결 이상 탐지
  - 서버가 출결 패턴 분석 및 이상 탐지 (AI 호출 포함)
- 수업/과목 성과 분석
  - 서버가 수업별/과목별 성과 분석 (AI 호출 포함)
- 지역 대비 부족 영역 분석
  - 서버가 지역 통계와 비교하여 부족 영역 식별 (AI 호출 포함)
- 월간 운영 리포트 생성
  - 서버가 월말 운영 리포트 생성 (AI 호출 포함)

**출력 예:**

"최근 4주간 월요일 지각률이 지역 평균보다 12% 높습니다.
등원 시간 재조정 및 수업 개편을 고려하세요."

**UI 노출 방식:**
- AI 인사이트는 기본 메뉴에서 항상 노출되며,
- 홈 화면 상단에 AI 브리핑 카드 자동 표시
- 기본 화면에서는 서버가 생성한 "요약 카드" 중심으로 (일부 타입에서만 AI 호출 발생)
- 사용자에게 부담 없이 인사이트를 제공하고,
- 상세 분석은 별도 페이지에서 제공한다.
- 즉, AI는 숨김 기능이 아니라 '가벼운 진입 → 깊은 분석' 구조의 핵심 메뉴이다.

**⚠️ AI 온오프 제어 (글로벌 헤더 토글) - SSOT 기준:**

- **위치**: 글로벌 헤더 우측 상단 (사용자 프로필/설정 메뉴 근처)
- **기능 (SSOT)**: `tenant_features(feature_key='ai').enabled` 값을 토글 (프론트 상황 신호 수집 문서 SSOT 섹션 참조)
- **동작**: AI 기능만 비활성화, 세부 페이지/메뉴는 숨기지 않음
- **즉시 반영**: 토글 변경 시 즉시 Edge Function에 전달
- **AI 비활성화 시**: Rule Engine 기반 폴백 처리 (AI 없이 규칙 기반 자동화만 수행)
- **시각적 피드백**: AI 브리핑 카드에 "AI 기능이 비활성화되어 있습니다" 메시지 표시 가능
...
→ 상세 구현은 전체 유아이문서 6-4 섹션 및 프론트 상황 신호 수집 문서 "글로벌 헤더 AI 토글 — UX/정책 SSOT" 섹션 참조
...
**⚠️ 🔥 2번: TaskCard / AI Insight / AI Briefing 규칙 통합 (경계 명확화, StudentTaskCard는 학생용 별칭):**
...
```typescript
card_type_boundary = {
  // TaskCard: "학생 단위 업무" 레이어 (StudentTaskCard는 학생용 별칭)
  TaskCard: {
...
card_creation_conflict_resolution = {
  // 결석 3일 → TaskCard만 생성 (AI Insight는 생성 안 함, StudentTaskCard는 학생용 별칭)
  'absence_3_days': {
    create: ['TaskCard'], // StudentTaskCard는 학생용 별칭
    skip: ['AI_Insight']  // 중복 방지
  },

  // 이탈 위험 → TaskCard 우선, AI Insight는 별도 생성 (StudentTaskCard는 학생용 별칭)
  'risk_detection': {
    create: ['TaskCard', 'AI_Insight'], // StudentTaskCard는 학생용 별칭
...
TaskCard, AI Insight, AI Briefing Card 세 시스템에 대한 통합 규칙 (StudentTaskCard는 학생용 별칭):
...
unified_card_dedup_ttl_priority_rule = {
  ...
  dedup: {
    ...
    same_type_same_entity_same_day: {
      ...
      TaskCard: 'same task_type + same entity_type + same entity_id + same day' + ' (StudentTaskCard는 학생용 별칭)',
  ...
  // 공통 TTL 규칙
  ttl: {
    TaskCard: 'expires_at 기준', // StudentTaskCard는 학생용 별칭
    AI_Insight: '24시간',
  ...
  // 공통 우선순위 계산
  priority: {
    TaskCard: '긴급도 + 시간대 + 담당 수업/학생', // StudentTaskCard는 학생용 별칭
...
**⚠️ 🔥 H1-1: DashboardCard 타입 정의 공통화 (SSOT 친화):**

**문제점:**
- `HomePage.tsx`와 `AllCardsPage.tsx`에서 동일한 타입 정의가 중복됨
- `EmergencyCard`, `AIBriefingCard`, `ClassCard`, `StatsCard`, `BillingSummaryCard` 인터페이스 및 `DashboardCard` 타입이 두 파일에 중복 존재
- 타입 변경 시 두 곳 모두 수정해야 하는 유지보수 문제

**해결 방안: 공통 타입 파일 추출**

**위치:** `apps/academy-admin/src/types/dashboardCard.ts`
...
// SSOT: Barrel Export를 통한 유틸리티 import
...
export function HomePage() {
  ...
  const sortedCards = React.useMemo(() => {
    ...
    // ...
    // ⚠️ 중요: 카드 생성 시 입력 정규화 레이어 사용 필수
    // created_at, priority, action_url 필드는 정규화 후 사용
...
**장점:**
- SSOT 원칙 준수: 타입 정의가 한 곳에만 존재
- 유지보수성 향상: 타입 변경 시 한 곳만 수정
- 타입 안정성: TypeScript로 타입 체크 가능
- 확장성: 새로운 카드 타입 추가 시 타입 파일에만 추가
...
home_dashboard_card_groups = {
  ...
  card_group_enum: [
    ...
    'CLASSES',        // 오늘의 수업 그룹
    'ATTENDANCE_STATS',  // 출석 관련 지표 그룹 (STATS 하위)
    'STUDENT_GROWTH_STATS',  // 학생 성장 지표 그룹 (STATS 하위)
    'REVENUE_STATS',  // 매출 관련 지표 그룹 (STATS 하위)
    'COLLECTION_STATS',  // 수납 관련 지표 그룹 (STATS 하위)
    'BILLING',        // 수납 요약 그룹
    'STATS'           // 통계/트렌드 그룹 (기타)
  ],
  ...
  within_group_priority: {
    ...
    STUDENT_TASKS: {
      // TaskCard의 priority 값 사용 (StudentTaskCard는 학생용 별칭)
      rule: 'use_card_priority',
      // 단, TaskCard priority가 90 이상이어도 Emergency 그룹보다는 낮음 (StudentTaskCard는 학생용 별칭)
  ...
  merge_rule: {
    ...
    // TaskCard priority가 90 이상이어도 Emergency 그룹보다는 낮음 (StudentTaskCard는 학생용 별칭)
    student_task_max_priority: 99,

    // AI Insight가 TaskCard에 의존하는 경우 상호 참조 방지 (StudentTaskCard는 학생용 별칭)
...
#### 3.7.1.1 홈 대시보드 카드 컴포넌트 공통화 (SSOT 친화)

**⚠️ 중요: HomePage와 AllCardsPage에서 카드 렌더링 로직 중복 제거**

**문제점:**
- `HomePage.tsx`와 `AllCardsPage.tsx`에서 각 카드 타입별 렌더링 로직이 중복됨
- Emergency Card, AI Briefing Card, Class Card, Stats Card, Billing Summary Card 렌더링이 두 파일에 중복 존재
- 카드 스타일 변경 시 두 곳 모두 수정해야 하는 유지보수 문제
...
**장점:**
- SSOT 원칙 준수: 카드 렌더링 로직이 한 곳에만 존재
- 유지보수성 향상: 카드 스타일 변경 시 컴포넌트 파일만 수정
- 재사용성: 다른 페이지에서도 동일한 카드 컴포넌트 사용 가능
- 테스트 용이성: 각 카드 컴포넌트를 독립적으로 테스트 가능

**공통화 대상 카드:**
- ✅ Emergency Card: 홈에서 항상 최상단 그룹으로 다루는 구조가 문서 예시에 존재
- ✅ AI Briefing Card: 홈 우선순위에 고정 등장 + AI OFF 시에도 메시지로 표시(숨김 금지)
- ✅ Class Card: 오늘의 수업 카드
- ✅ Stats Card: 통계 카드
- ✅ Billing Summary Card: 청구 요약 카드

**⚠️ 참고:**
- StudentTaskCard는 이미 공통 컴포넌트로 분리되어 있음 (`apps/academy-admin/src/components/StudentTaskCard.tsx`)
- StudentTaskCard만 공통 분리된 이유: AI Suggestion 개념 폐기 및 StudentTaskCard로 정본 통합, 승인(L2)·미리보기·감사로그 등 운영/리스크 요구사항이 컴포넌트 중심으로 결속

#### 3.7.1.2 대시보드 카드 공통화 전략 (Phase 2 시각적 개선)
...
**Daily briefing: 07:00 서버가 생성 (AI 호출 포함)**
- 매일 아침 7시에 전날 데이터 기반 브리핑 서버가 생성 (AI 호출 포함)
...
**상담일지 요약: 저장 시 즉시 생성**
- 상담일지 저장 시 서버가 AI 요약 생성 (Behavior-based Event)
- Level 3 Auto-Action으로 자동 실행

#### 3.7.3 AI Insight 정확도 보정(Weighting Rule)

서버가 AI Insight의 여러 지표를 합산할 때 가중치를 적용한다.
...
서버가 AI Risk Score를 계산할 때 반드시 업종별(`industry_type`) 기본 가중치를 사용하며, Tenant Override가 우선 적용됩니다.
...
- 서버가 AI 학습 결과에 따라 가중치 조정 가능

**업종별 기본 가중치 차등:**

서버가 AI Risk Model 가중치를 적용할 때 `industry_type`별로 Default Policy가 다르며, `tenant_override` 가능함을 명시한다:
...
```sql
-- ⚠️ 레거시: students 테이블은 레거시이며, 신규 코드에서는 사용 금지. 정본은 persons + academy_students 조합이며, 신규 코드는 persons + academy_students 조합만 사용
...
**2) TaskCard Override** (⚠️ 참고: 'Suggestion Override'는 레거시 용어, StudentTaskCard는 학생용 별칭)
- TaskCard (task_type: 'ai_suggested', entity_type='student') 메시지 초안 수정 → 새로운 템플릿 학습 (StudentTaskCard는 학생용 별칭)
- 추천된 메시지 내용 수정 및 재발송
- TaskCard (task_type: 'ai_suggested', entity_type='student') 무시 옵션 (StudentTaskCard는 학생용 별칭)
...
- 피드백 유형: `insight_override`, `student_task_override` (⚠️ 참고: 'suggestion_override'는 레거시, 현재는 TaskCard 피드백, StudentTaskCard는 학생용 별칭), `auto_action_undo`
- 피드백 내용: 원래 AI 판단, 사용자 보정 내용, 보정 이유

**피드백 재반영:**
- 주기적 모델 재학습 시 `ai_feedback_logs` 데이터를 학습 데이터에 포함
- Override된 패턴은 향후 동일 상황에서 서버가 AI 모델을 재학습하여 다른 추천을 하도록 가중치 조정
...
| 서버가 상담 AI 요약 생성 | ⭕ 가능 | 요약 재생성 가능 | 요약 삭제 후 재생성 |
...
| ai_type | VARCHAR | insight / auto_action (⚠️ 참고: 'suggestion'은 레거시, 현재는 TaskCard 사용, StudentTaskCard는 학생용 별칭) |
...
- Level 2 (TaskCard, task_type: 'ai_suggested', entity_type='student'): TaskCard 피드백 모델에 반영 (⚠️ 참고: 'AI Suggestion'은 레거시 용어, StudentTaskCard는 학생용 별칭)
...
사용자가 클릭하지 않아도 시스템이 먼저 추천/분석을 수행합니다. 실행(발송/적용)은 승인/정책 게이트를 통해 이루어집니다.
...
TaskCard의 priority는 'TaskCard 그룹 내부'에서만 적용된다. (StudentTaskCard는 학생용 별칭)
...
- 긴급 알림은 항상 최상단 고정
- 동일 우선순위 내에서는 시간순 정렬
- 카드 개수 제한: 제한 없음 (모든 카드 출력, 그룹별로 분류하여 표시)
...
- AI 그룹은 Emergency 다음
- TaskCard 그룹 내부에서만 priority 필드로 정렬 (StudentTaskCard는 학생용 별칭)
- emergency_card 조건이 TaskCard의 priority보다 강함 (StudentTaskCard는 학생용 별칭)

**⚠️ M8: Home 화면 카드 출력 규칙 (v3.4 업데이트, StudentTaskCard는 학생용 별칭):**

**카드 출력 규칙 (v3.4 확정):**

홈 화면은 모든 카드를 출력하며, 그룹별로 분류하여 4열 그리드 레이아웃으로 표시한다:

```typescript
home_card_display = {
  // 카드 출력 제한: 없음 (모든 카드 출력)
  card_limit: null,
  ...
  layout: {
    ...
    tablet_columns: 2,
    mobile_columns: 1,
    group_by_category: true  // 그룹별로 분류하여 표시
  },
  ...
  card_groups: {
    ...
    STUDENT_TASKS: '학생 업무',
    CLASSES: '오늘 수업',
    ATTENDANCE_STATS: '출석 관련 지표',
    STUDENT_GROWTH_STATS: '학생 성장 지표',
    REVENUE_STATS: '매출 관련 지표',
    COLLECTION_STATS: '수납 관련 지표',
    BILLING: '수납 현황'
  },
...
```typescript
emergency_rule = {
  // 결제 실패가 최근 N일간 2회 이상 발생 (Policy 기반 lookback window)
  // Policy 경로: auto_notification.recurring_payment_failed.threshold (임계값, 건수)
  // Policy 경로: auto_notification.recurring_payment_failed.lookback_days (조회 기간, 일수)
...
| **수업 배정** | SDUI + Custom | Table + Custom Action |
...
direct_update_allowed = {
  // Default Policy: Direct Update는 테넌트 생성 시 비활성으로 설정값으로 저장
  default_policy: {
    direct_update_enabled: false,  // Default Policy: 테넌트 생성 시 false로 설정값으로 저장
...
- 실시간성이 중요한 경우: Direct Update (허용 테이블/필드만, RLS 정책 필수, Default Policy에 따라 비활성으로 설정)
- 출결/청구/AI 관련 테이블: Direct Update 절대 금지, Edge Function 필수

**⚠️ Implementation 가이드:**
- Direct Update는 Default Policy에 따라 비활성으로 설정됨
...
- 모든 이벤트는 `event_logs` + 내부 Queue에 적재 후 비동기 처리
  - ⚠️ 참고: `event_logs.event_type`은 자동화 카탈로그의 `event_type`과 다른 도메인 값입니다. `event_logs.event_type`은 시스템 이벤트 타입(예: `payment_webhook`, `attendance.check_in`)이며, 자동화 카탈로그의 `event_type`은 자동화 시나리오 키(예: `overdue_outstanding_over_limit`, `payment_due_reminder`)입니다.
...
- ⚠️ 참고: automation_actions(워크플로우 이벤트)와 execution_audit_runs(실행 결과)의 관계는 액티비티.md 참조
...
**시스템 이벤트 Role 정의:**
Time-based 서버가 청구 생성, Webhook 이벤트 등 시스템이 직접 생성하는 이벤트는 `system_role = 'system'`으로 평가된다.
...
ai_feedback_event_schedule_alignment = {
  ...
  event_engine_schedule: {
    '04:00': 'auto_billing_generation',  // 서버가 청구 생성
...
counseling_note_saved (by Teacher)
  ...
  → UI (TaskCard 업데이트, StudentTaskCard는 학생용 별칭)
```
...
- 출결 이상 패턴 감지 시 서버가 위험도 분석 후 알림 발송 결정 (AI 호출 포함)
...
- 원장(Admin)만 서버가 청구 생성, 미납 알림 실행 가능
...
**⚠️ 시스템이 생성한 이벤트의 Role 정의:**

Time-based 서버가 청구 생성, Webhook 이벤트 등 시스템이 생성한 이벤트는 role이 없으므로 별도 처리 규칙이 필요하다:

```typescript
system_event_role = {
  // 시스템이 생성한 이벤트는 'system' role로 처리
  system_role: 'system',
...
**시스템 이벤트 처리:**
- Time-based 서버가 청구 생성: `role='system'` → role 검증 우회
...
문서 전반에서 사용하는 `attendance.*`, `location.*` 등 경로는 모두 tenant_settings(key='config').value(JSONB) 내부 경로이다. 이 구조는 Schema 설계 시 단일 소스로 관리된다.

**tenant_settings KV 구조의 value(JSONB) 스키마 v1 공식 정의:**

⚠️ 중요: tenant_settings는 KV 구조이며, 모든 설정은 key='config'인 단일 row의 value(JSONB) 필드에 저장됩니다.
아래는 value(JSONB) 내부의 단일 JSON 스키마로 통합 관리됩니다:
...
위에서 정의된 Event Engine 시간 기반 스케줄(04:00 서버가 청구 생성, 07:00 서버가 AI 데일리 브리핑 생성, 03:00 AI Feedback 등)은
...
| class_attendance_heatmap | TABLE (Staging) | 수업별 출결 히트맵 |
| analytics.daily_store_metrics | TABLE | 매장 단위 일별 KPI (정본) |
| analytics.daily_region_metrics | TABLE | 지역 단위 집계 KPI (정본) |
| ai_insights | TABLE | AI 인사이트 논리 테이블 (필요 시 MV로 파생 가능) |

**⚠️ 참고:**
- ~~regional_metrics_daily~~, ~~tenant_kpi_daily~~는 구버전/폐기된 네이밍입니다.
- 정본 테이블은 `analytics.daily_store_metrics`, `analytics.daily_region_metrics`입니다.
- 성능 최적화가 필요한 경우 MV를 생성할 수 있으나, 정본 테이블을 우선 사용합니다.
...
- 04:00 서버가 청구 생성
- 07:00 서버가 AI 데일리 브리핑 생성 (AI 호출 포함)
...
**상용화 단계: 단일 Region + 정기 백업**
- Supabase 자동 백업 활용
- 일일 자동 백업
- PITR(Point-In-Time Recovery) 기능 활용
- 백업 보관 기간 설정

**상용화 단계: Read Replica 기반 보고서 분리**
- 읽기 전용 쿼리를 Read Replica로 분리
- 보고서/통계 조회 성능 향상
- 메인 DB 부하 감소

**선택적 구현: 멀티 리전/DR (외부 DB 전환 필요 시)**
- Region Failover 전략
- 상세 RTO/RPO 정책
- 자동 장애 복구 메커니즘

**상세 정책:**
- T7 참조 (Multi-Region DR은 선택적 구현으로 외부 DB 기반으로 구현, 실제 필요 시)
...
Regional Analytics는 `location.*` 경로가 필수이므로, 테넌트 생성 시 위치 정보를 반드시 등록해야 합니다 (저장 위치는 tenant_settings(key='config').value(JSONB)):
...
**Step 5: 지역 코드 저장**
- `location.*` 경로에 모든 위치 정보 저장 (저장 위치는 tenant_settings(key='config').value(JSONB)):
  ```json
  {
    "location": {
      "si": "서울특별시",
...
- students: 학생 정보
- classes: 수업 정보
- attendance_logs: 출결 로그 (⚠️ B6: GPS/Auth Method 필드 포함)
...
```json
{
  ...
  "event_type": "payment_webhook",  // ⚠️ 참고: event_logs.event_type은 자동화 카탈로그의 event_type과 다른 도메인 값입니다
  "tenant_id": "uuid",
  "payload": {
    "invoice_id": "uuid",
    "amount": 300000,
...
- Partial Payment: 부분납부, 청구 금액의 일부만 납부
- Early Payment: 조기납부, 기간 내 조기 납부 시 할인
- TaskCard: 업무 카드, 서버가 생성하며 일부 타입에서만 AI 호출 발생 (StudentTaskCard는 학생용 별칭)
...
출결 설정 JSON은 **3.3.7 출결 설정** 섹션의 `attendance.*` 경로 공식 스펙만 유지하고, 다른 섹션에서는 참조 형태로만 언급 (저장 위치는 tenant_settings(key='config').value(JSONB)):
...
위의 `attendance.*` 경로 공식 스펙 (단일 소스) 섹션(3.3.7 출결 설정, 1732-1776줄)이 유일한 정식 정의입니다. (저장 위치는 tenant_settings(key='config').value(JSONB))
...
## 13. Phase 3: Industry-Based Routing & Terminology (업종별 라우팅 및 용어 체계)

### 13.1 개요 (Overview)

Phase 3는 플랫폼의 **업종 중립성(Industry Neutrality)**을 UI/UX 레벨까지 완전히 구현하는 프로젝트입니다.

**핵심 목표:**
- 모든 하드코딩된 업종 특화 용어(학생, 수업, 강사 등)를 동적 용어로 전환
- 업종별로 필요한 페이지만 표시 (VISIBLE_PAGES)
- 단일 코드베이스로 5개 업종(Academy, Gym, Salon, Nail Salon, Real Estate) 지원
- Industry Registry를 SSOT(Single Source of Truth)로 확립

**불변 원칙:**
- ❌ 하드코딩된 용어 사용 금지 ("학생", "수업", "강사" 등)
- ✅ Industry Registry를 통한 동적 용어 참조만 허용
- ❌ 업종별 중복 코드/컴포넌트 생성 금지
- ✅ 업종별 차이는 Industry Adapter 레이어에서만 허용

### 13.2 Industry Registry (SSOT)

**위치:** `packages/industry/industry-registry.ts`
...
export interface IndustryTerms {
  ...
  PERSON_TYPE_PRIMARY: string;        // DB 저장용 (예: 'student')
  PERSON_LABEL_PRIMARY: string;       // UI 표시용 (예: '학생')
  PERSON_LABEL_PLURAL: string;        // 복수형 (예: '학생들')

  // 보조 관리 대상 (Secondary Person)
  PERSON_TYPE_SECONDARY: string;      // 예: 'teacher'
  PERSON_LABEL_SECONDARY: string;     // 예: '강사'
  PERSON_LABEL_SECONDARY_PLURAL: string;

  // 그룹/클래스 개념
  GROUP_TYPE: string;                 // 예: 'class'
  GROUP_LABEL: string;                // 예: '수업'
  GROUP_LABEL_PLURAL: string;         // 예: '수업들'

  // 출석 관련
  ATTENDANCE_LABEL: string;           // 예: '출결' (academy), '출석' (gym), '방문' (salon)
  ABSENCE_LABEL: string;              // 예: '결석'
  LATE_LABEL: string;                 // 예: '지각'
  PRESENT_LABEL: string;              // 예: '출석'
  EXCUSED_LABEL: string;              // 예: '사유'
  CHECK_IN_LABEL: string;             // 예: '등원' (academy), '입장' (gym)
  CHECK_OUT_LABEL: string;            // 예: '하원' (academy), '퇴장' (gym)
  TOTAL_LABEL: string;                // 예: '총원'
  SESSION_LABEL: string;              // 예: '수업' (academy), '세션' (gym), '시술' (salon)

  // 상담/컨설팅
  CONSULTATION_LABEL: string;         // 예: '상담'
  CONSULTATION_LABEL_PLURAL: string;

  // 통계 카드 제목
  STATS_TOTAL_COUNT_TITLE: string;    // 예: '총 학생 수'
  STATS_NEW_THIS_MONTH_TITLE: string; // 예: '이번 달 신규 등록'
  ...
  // 라우팅 경로
  ROUTES: {
    PRIMARY_LIST: string;              // 예: '/students/home'
    PRIMARY_RISK: string;              // 예: '/students?filter=risk'
    PRIMARY_ABSENT: string;
    PRIMARY_CONSULTATION: string;
    CLASSES: string;                   // 예: '/classes'
    TEACHERS: string;                  // 예: '/teachers'
    APPOINTMENTS?: string;             // 예: '/appointments' (salon, real_estate)
  };
...
export interface VisiblePages {
  primary: boolean;          // 주요 관리 대상 페이지 (학생/회원/고객)
  attendance: boolean;       // 출석 관리 (academy, gym만 true)
  classes: boolean;          // 수업/서비스 관리
  teachers: boolean;         // 강사/스태프 관리
  billing: boolean;          // 수납/결제 관리
  analytics: boolean;        // 통계 분석
  ai: boolean;               // AI 기능
  automation: boolean;       // 자동화 설정
  alimtalk: boolean;         // 알림톡 설정
  appointments?: boolean;    // 예약 관리 (salon, nail_salon, real_estate만 true)
  properties?: boolean;      // 매물 관리 (real_estate만 true)
}
```

**지원 업종:**

| 업종 | industry_type | 주요 관리 대상 | 그룹 개념 | 출석 개념 | 비고 |
|------|--------------|--------------|----------|----------|------|
| 학원 | academy | 학생 (student) | 수업 (class) | 출결 | 기본 업종 |
| 체육관 | gym | 회원 (member) | 그룹 (group) | 출석 | 헬스장, PT센터 |
| 미용실 | salon | 고객 (customer) | 서비스 (service) | 방문 | 예약 중심 |
| 네일샵 | nail_salon | 고객 (customer) | 서비스 (service) | 방문 | 예약 중심 |
| 부동산 | real_estate | 고객 (client) | 매물 (property) | 방문 | 매물 관리 |

**API:**

```typescript
// Industry Registry에서 용어 가져오기
...
console.log(terms.GROUP_LABEL);          // '수업'

const gymTerms = getIndustryTerms('gym');
console.log(gymTerms.PERSON_LABEL_PRIMARY); // '회원'
console.log(gymTerms.GROUP_LABEL);          // '그룹'
```

### 13.3 React Hooks for Industry Terms

**위치:**
- `packages/hooks/use-industry-terms/src/useIndustryTerms.ts`
- `packages/hooks/use-industry-config/src/useIndustryConfig.ts`

#### 13.3.1 useIndustryTerms Hook

**용도:** 현재 테넌트의 업종별 용어를 가져옴

**Zero-Trust 원칙:**
- tenantId는 항상 Context에서 추출 (파라미터로 전달하지 않음)
- 인증되지 않은 상태에서는 fallback으로 'academy' 반환
...
#### 13.3.2 useIndustryConfig Hook

**용도:** 용어 + 페이지 가시성 체크 기능 제공
...
function App() {
  ...
  // salon, nail_salon은 attendance가 false이므로 메뉴에 표시 안 됨

  return <Sidebar items={menuItems} />;
}
```

#### 13.3.3 한국어 조사 유틸리티 (Korean Particle Utils)

**위치:** `apps/academy-admin/src/utils/korean-particle-utils.ts`

**용도:** 업종 중립 용어에 올바른 한국어 조사 적용

업종에 따라 용어가 동적으로 변경되므로 (학생/회원/고객), 조사도 받침 유무에 따라 동적으로 적용해야 합니다.
...
`${terms.TAG_LABEL}${p.을를(terms.TAG_LABEL)} 삭제하시겠습니까?`
// "태그를 삭제하시겠습니까?"

// templates 사용 (자주 쓰는 문장 패턴)
...
**menuLabels 서브사이드바 라벨:**
- `menuLabels.list(entity)`: "{entity}목록"
- `menuLabels.management(entity)`: "{entity}관리"
- `menuLabels.statistics(entity)`: "{entity}통계"

### 13.4 IndustryBasedRoute Component

**위치:** `apps/academy-admin/src/components/IndustryBasedRoute.tsx`

**용도:** 업종별로 접근 불가능한 페이지로 라우팅 시 자동 리다이렉트

**구조:**

```typescript
interface IndustryBasedRouteProps {
  page: keyof VisiblePages;  // 'classes' | 'attendance' | 'appointments' 등
  children: React.ReactNode;
}

function IndustryBasedRoute({ page, children }: IndustryBasedRouteProps) {
...
```tsx
<Routes>
  {/* ✅ attendance 페이지: academy, gym만 접근 가능 */}
  ...
  {/* ✅ appointments 페이지: salon, nail_salon, real_estate만 접근 가능 */}
...
**동작:**
- Academy 테넌트가 `/appointments` 접근 시 → `/home`으로 리다이렉트
- Salon 테넌트가 `/attendance` 접근 시 → `/home`으로 리다이렉트

### 13.5 페이지별 적용 사례

#### 13.5.1 ClassesPage (수업/서비스 관리)
...
export function ClassesPage() {
  ...
  return (
    <>
      <PageHeader title={`${terms.GROUP_LABEL} 관리`}>
        <Button>{terms.GROUP_LABEL} 생성</Button>
      </PageHeader>
      ...
      <ClassList>
        {classes.map(cls => (
          <ClassCard
            title={cls.name}
            teacherLabel={terms.PERSON_LABEL_SECONDARY}
            studentLabel={terms.PERSON_LABEL_PRIMARY}
...
**결과:**
- Academy: "수업 관리", "수업 생성", "강사: 김선생님", "학생 15명"
- Gym: "그룹 관리", "그룹 생성", "트레이너: 김코치님", "회원 15명"
- Salon: "서비스 관리", "서비스 생성", "스타일리스트: 김원장님", "고객 15명"

#### 13.5.2 NotificationsPage (메시지 발송)
...
function App() {
  ...
  const getSidebarItemsForRole = (role: TenantRole) => {
    const menuItems = [
      { id: 'home', label: '대시보드', path: '/home' },
    ];
...
**위치:** `apps/academy-admin/vite.config.ts`

**Industry Registry 별칭 설정:**
...
export default defineConfig({
  resolve: {
    alias: [
      {
        find: '@industry/registry',
        replacement: path.resolve(__dirname, '../../packages/industry/industry-registry.ts')
      },
      // ... other aliases
    ],
  },
...
**중요:** `@industry/registry`는 단일 파일이므로 직접 파일 경로로 매핑합니다.

### 13.7 구현 완료 현황

**✅ 완료된 페이지:**
1. HomePage (대시보드)
2. StudentsPage (학생/회원/고객 관리)
3. ClassesPage (수업/그룹/서비스 관리)
4. TeachersPage (강사/트레이너/스태프 관리)
5. AttendancePage (출결/출석/방문 관리)
6. NotificationsPage (메시지 발송)
7. AnalyticsPage (통계 분석)
8. AIPage (인공지능)
9. AutomationSettingsPage (자동화 설정)
10. AlimtalkSettingsPage (알림톡 설정)
11. BillingPage (수납 관리)

**✅ 완료된 컴포넌트:**
- IndustryBasedRoute (라우팅 보호)
- Industry Registry (SSOT)
...
**⚠️ 미완료 (낮은 우선순위):**
- LoginPage, SignupPage (인증 페이지는 업종 중립)

**✅ 2026-01-17 완료:**
- students/tabs/* (학생 상세 탭) → StudentInfoTab, GuardiansTab, ConsultationsTab, TagsTab, ClassesTab, AttendanceTab, RiskAnalysisTab, MessageSendTab
- students/subpages/* (SubSidebar 탭별 SubPage) → StudentListSubPage, StudentConsultSubPage, StudentTagsSubPage, StudentStatsSubPage
- 캐시 키 SSOT (cache-keys.ts) → STUDENT_CACHE_KEYS, invalidateStudentConsultations 등

### 13.8 확장 가이드

**새로운 업종 추가 시:**

1. **Industry Registry에 용어 정의 추가:**
```typescript
const NGO_TERMS: IndustryTerms = {
  PERSON_TYPE_PRIMARY: 'beneficiary',
  PERSON_LABEL_PRIMARY: '수혜자',
  GROUP_TYPE: 'program',
  GROUP_LABEL: '프로그램',
  ATTENDANCE_LABEL: '참여',
  // ... 모든 필드 정의
  ...
  VISIBLE_PAGES: {
    ...
    classes: true,
    teachers: true,
    billing: false,  // NGO는 수납 기능 불필요
    // ...
  },
};

export const INDUSTRY_TERMS: Record<IndustryType, IndustryTerms> = {
  academy: ACADEMY_TERMS,
  gym: GYM_TERMS,
  salon: SALON_TERMS,
  nail_salon: NAIL_SALON_TERMS,
  real_estate: REAL_ESTATE_TERMS,
  ngo: NGO_TERMS,  // 신규 업종 추가
};
```

2. **코드 변경 불필요:**
- 모든 페이지는 이미 Industry Registry를 참조하므로 자동 적용
- VISIBLE_PAGES 설정에 따라 메뉴/라우팅 자동 필터링

### 13.9 금지 패턴 (Anti-Patterns)
...
```typescript
// ✅ Industry Registry를 통한 동적 용어
const terms = useIndustryTerms();
<PageHeader title={`${terms.PERSON_LABEL_PRIMARY} 관리`} />
<Button>{terms.GROUP_LABEL} 생성</Button>
<Badge>{terms.ATTENDANCE_LABEL} 기반</Badge>

// ✅ 단일 컴포넌트 + Industry Adapter
<PrimaryPersonManagementPage terms={terms} />

// ✅ VISIBLE_PAGES를 통한 자동 필터링
{isPageVisible('attendance') && <AttendanceMenuItem />}
```

### 13.10 관련 문서 참조

- **1.2.1 Domain/Service 전체 목록 (업종 독립)**: Core vs Industry Layer 구분
- **55-100줄 Automation & AI Industry-Neutral Rule**: 자동화/AI의 업종 독립성 원칙
- **94-147줄 Edge Functions Industry Adapter**: 서버 사이드 업종별 테이블명 매핑
- **148-163줄 AI UI 컴포넌트 업종 독립 설계**: UI 컴포넌트 업종 확장 포인트

### 13.11 검증 체크리스트

Phase 3 구현 완료 시 다음 항목을 검증해야 합니다:

- [ ] 모든 페이지에서 하드코딩된 업종 특화 용어 제거 확인
- [ ] Industry Registry의 모든 필드가 5개 업종에 대해 정의됨
- [ ] VISIBLE_PAGES 설정이 올바르게 적용되어 메뉴 필터링 작동
- [ ] IndustryBasedRoute가 접근 불가 페이지 차단
- [ ] TypeScript 컴파일 0 errors
- [ ] 빌드 성공 (Bundle Size < 500KB)
- [ ] 런타임 테스트: academy → gym → salon 전환 시 UI 동적 변경 확인

---

## 14. 변경 이력 (Changelog)

- 2026-01-17: StudentsPage 탭 분리 및 캐시 SSOT 구현
  - StudentsPage SubSidebar 탭별 SubPage 컴포넌트 분리 (StudentListSubPage, StudentConsultSubPage, StudentTagsSubPage, StudentStatsSubPage)
  - 학생 관련 캐시 키 SSOT 생성 (`packages/hooks/use-student/src/cache-keys.ts`)
  - STUDENT_CACHE_KEYS 상수 및 invalidateStudentConsultations 등 캐시 무효화 유틸리티 함수 추가
  - 상담 mutation에 전체 상담 캐시 무효화 추가 (RightLayerMenu ↔ SubSidebar 동기화 해결)
- 2026-01-04: Phase 3: Industry-Based Routing 구현 완료
  - Industry Registry (SSOT) 구현 (`packages/industry/industry-registry.ts`)
  - IndustryTerms 인터페이스 정의 (59개 필드, 5개 업종)
  - useIndustryTerms, useIndustryConfig Hook 구현
  - IndustryBasedRoute 컴포넌트 구현
  - 11개 주요 페이지에 동적 용어 적용 (ClassesPage, TeachersPage, NotificationsPage 등)
  - App.tsx 사이드바 메뉴 업종별 필터링 구현
  - VISIBLE_PAGES 기반 페이지 가시성 제어
  - TypeScript 빌드 오류 0건 (ATTENDANCE_LABEL, SESSION_LABEL 추가)
  - Bundle Size: 482.26 KB / 500.00 KB (96.5%)
- YYYY-MM-DD: AI 온오프 SSOT 통일 작업 완료
  - AI 온오프 제어를 `tenant_features(feature_key='ai').enabled`로 통일 (SSOT)
  - `tenant_settings.ai.enabled`는 레거시로 표시
  - 플랫폼 스위치 `PLATFORM_AI_ENABLED` 추가 (env-registry/server)
  - 최종 유효값 계산 로직 명시: `effective_ai_enabled = PLATFORM_AI_ENABLED && tenant_features['ai'].enabled`
  - 프론트 상황 신호 수집 문서 SSOT 섹션 참조 추가
- YYYY-MM-DD: Critical 이슈 수정 완료
  - Analytics 테이블명 정본화 (regional_metrics_daily → analytics.daily_region_metrics, SSOT 확정)
  - 구버전 네이밍 완전 제거 (analytics.daily_metrics, analytics.monthly_revenue, regional_metrics_daily)
  - 정본 테이블 명시: analytics.daily_store_metrics, analytics.daily_region_metrics (SSOT)
  - AI 인사이트 엔티티 단일화 (ai_insights 정본, MV는 파�
