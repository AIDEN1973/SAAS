...
- ✅ 업종 중립적 용어로 변경 (학생→구성원, 강사→담당자, 반→그룹 등)
...
unified_card_dedup_ttl_priority_rule = {
  ...
  // 공통 우선순위 계산
  priority: {
    TaskCard: '긴급도 + 시간대 + 담당 반/학생', // StudentTaskCard는 학생용 별칭
...
**핵심 목표:**
- 모든 하드코딩된 업종 특화 용어(학생, 반, 강사 등)를 동적 용어로 전환
...
|------|--------------|--------------|----------|----------|------|
| 학원 | academy | 학생 (student) | 반 (class) | 출결 | 기본 업종 |
...
console.log(terms.GROUP_LABEL);          // '반'
...
**결과:**
- Academy: "반 관리", "반 생성", "강사: 김선생님", "학생 15명"
