...
데이터 보존 정책 및 파티션 전략 추가

P0-15: 데이터 보존 정책 법적 근거 명시 (개인정보보호법 2년, 국세기본법 5년)
P0-16: 파티션 전략 정본화 (2025-2075년, 51년 범위)
P0-17: 차등 보존 정책 명시 (일반 로그 2년, 회계 로그 5년)

18. 데이터 보존 정책 및 파티션 전략 (SSOT)

18.1 법적 근거 및 보존 기간

**execution_audit_runs (실행 감사 로그)**
...
법적 근거:
- 개인정보보호법: 2년 최소 보존 의무
- 국세기본법: 5년 최소 보존 의무 (회계 관련 기록)

회계 관련 operation_type:
- send-invoice (청구서 발송)
...
정리 메커니즘:
- 함수: `cleanup_old_execution_audit_runs(730, 1825)`
- pg_cron: 매일 03:00 KST (18:00 UTC) 자동 실행
- 차등 삭제: operation_type에 따라 2년/5년 기준 적용

**execution_audit_steps (실행 감사 세부 단계)**

보존 기간: 상위 run과 동일 (CASCADE 자동 삭제)

법적 근거: 상위 execution_audit_runs 정책 준수

정리 메커니즘:
- run 삭제 시 자동 삭제 (CASCADE)
- Orphaned steps 정리: `cleanup_orphaned_execution_audit_steps()`

18.2 파티션 전략 (2025-2075, 51년)

**파티션 범위**

execution_audit_runs: 2025-2075년 (51년, 연도별 파티션)
execution_audit_steps: 2025-2075년 (51년, 연도별 파티션)

**파티션 전략 근거**

장기 보존 요구 대비:
- 법적 분쟁 발생 시 과거 데이터 복원 가능
- 규제 변경 대응 여유 확보 (예: 보존 기간 연장)

운영 안정성:
- 2075년까지 파티션 관리 불필요
- 파티션 누락으로 인한 데이터 삽입 실패 방지

비용 효율성:
- 빈 파티션 비용: 약 24-32KB (메타데이터만)
- 51년 파티션 × 2 테이블 = 102개 파티션 ≈ 3MB
- Partition Pruning으로 쿼리 성능 최적화

**파티션 명명 규칙**

execution_audit_runs_YYYY (예: execution_audit_runs_2025)
execution_audit_steps_YYYY (예: execution_audit_steps_2025)

범위: YYYY-01-01 ~ (YYYY+1)-01-01

**인덱스 전략**

execution_audit_runs 파티션별 인덱스:
- idx_execution_audit_runs_YYYY_tenant_occurred: (tenant_id, occurred_at DESC)
- idx_execution_audit_runs_YYYY_operation: (tenant_id, operation_type, occurred_at DESC)
- idx_execution_audit_runs_YYYY_status: (tenant_id, status, occurred_at DESC)

execution_audit_steps 파티션별 인덱스:
- idx_execution_audit_steps_YYYY_tenant_occurred: (tenant_id, occurred_at DESC)
- idx_execution_audit_steps_YYYY_run: (tenant_id, run_id)
- idx_execution_audit_steps_YYYY_status: (tenant_id, status, occurred_at DESC)
...
- 불필요한 파티션 자동 제외
- 51개 파티션 중 1-2개만 스캔
- 쿼리 성능 향상 및 인덱스 크기 감소

18.3 보존 정책 운영 규칙

**자동 정리 스케줄**

모든 정리 작업: 매일 03:00 KST (18:00 UTC)

스케줄 이름: `cleanup_execution_audit_runs_daily`
...
권한: super_admin 또는 service_role

함수:
```sql
SELECT public.admin_cleanup_execution_audit_runs(
  p_general_retention_days := 730,
  p_financial_retention_days := 1825
);
```
...
{
  "success": true,
  "deleted_general": 1234,
  "deleted_financial": 567,
  "general_retention_days": 730,
  "financial_retention_days": 1825,
...
⚠️ 중요: 보존 기간 변경 시 법적 근거 확인 필수

절차:
1. 법적 근거 검토 (법무팀 협의)
2. Migration 파일 생성 (함수 파라미터 변경)
3. pg_cron 스케줄 업데이트
4. 변경 이력 문서화 (본 문서 업데이트)

예시:
```sql
-- 일반 로그 2년 → 3년 변경 (법적 근거 필요)
CREATE OR REPLACE FUNCTION public.cleanup_old_execution_audit_runs(
  general_retention_days int DEFAULT 1095,   -- 3년
  financial_retention_days int DEFAULT 1825  -- 5년 (불변)
)
-- ...
...
SELECT cron.schedule(
  ...
  '0 18 * * *',
  'SELECT public.cleanup_old_execution_audit_runs(1095, 1825)'
);
```
...
초기 파티션: 2025-2032년 (8년)
- Migration: `20260112000010_create_execution_audit_partitions_and_retention.sql`
- Migration: `20260112000011_create_execution_audit_steps_partitions_and_retention.sql`

확장 파티션: 2033-2075년 (43년 추가)
- Migration: `20260112000014_extend_partitions_to_2075.sql`
- 총 51년 범위 (2025-2075)
...
예상 결과: 각 테이블당 51개 파티션

**파티션 추가 절차 (필요 시)**

⚠️ 주의: 51년 범위 설정으로 2075년까지 추가 불필요

2075년 이후 추가 필요 시:
1. Python 스크립트 실행 (`generate_extended_partitions.py`)
2. 생성된 SQL 파일 검토
...
**마이그레이션 파일**
- `infra/supabase/supabase/migrations/20260112000010_create_execution_audit_partitions_and_retention.sql`
- `infra/supabase/supabase/migrations/20260112000011_create_execution_audit_steps_partitions_and_retention.sql`
- `infra/supabase/supabase/migrations/20260112000014_extend_partitions_to_2075.sql`

**검증 보고서**
- `infra/supabase/PARTITION_VERIFICATION_REPORT.md` (2026-01-12 검증 완료)

**관련 SSOT 문서**
- `챗봇.md` 6.3: automation_actions 테이블 정본 (워크플로우 이벤트 기록)
- `프론트 자동화.md` 2.5: automation_actions 보존 정책 (2년, 730일) �
