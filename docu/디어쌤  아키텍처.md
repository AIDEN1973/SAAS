...
- `/ui-core` → `packages/core-ui/*` (UI Composition 계층)
...
/packages
  ...
  /core-ui/*              # UI Composition 계층
  /design-system          # Atlaskit 기반 공통 컴포넌트 매핑
...
- 스키마엔진 문서에서는 schema-engine → @atlaskit/components → @atlaskit/theme 의존성 방향까지 명시
...
- 의존성 방향: schema-engine → @atlaskit/components → @atlaskit/theme (S3 참조)

**핵심 제약:**
- **Tailwind 클래스 직접 사용 금지**: 어떤 레이어에서도 Tailwind 클래스를 직접 사용하지 않는다
- **Atlaskit 기반 컴포넌트**: 실제 컴포넌트는 Atlaskit 기반이며, design-system 레이어를 통해서만 사용한다
