...
학생 메뉴 첫 진입 시 라우트는 `/students/home`이며, 기본 화면은 항상 오늘의 학생 업무 카드(TaskCard, entity_type='student')이다. (StudentTaskCard는 학생용 별칭)

전체 학생 목록은 `/students/list`로 분리되며, 홈 화면에서는 별도의 "전체 학생 보기" 버튼으로만 진입 가능하다.

**업무 중심 + AI 중심 방식**

학생관리 첫 화면은 리스트가 아니라 "오늘의 학생 업무 카드(**서버가** 생성)"를 노출한다.

**기본 화면 구성 (업무 카드 중심)**

**[상담 필요 학생 2명]**
- 서버가 상담일지 패턴 분석으로 추천 생성 (AI 호출 후 서버가 정책 해석하여 실행)
- 카드 클릭 시 해당 학생 상세 + 상담일지 작성 화면으로 이동 (사용자 클릭 필요)

**[최근 3일 결석 학생 1명 – 부모 연락 필요]**
- 서버가 결석 패턴 분석으로 추천 생성 (AI 호출 후 서버가 정책 해석하여 실행)
- 카드 클릭 시 학부모 연락 화면 + AI 추천 메시지 초안 제공

**[성적/패턴 기반 이탈 위험 학생 1명]**
- 서버가 출결·성적·상담 패턴 종합 분석 (AI 호출 포함)
...
각 카드를 클릭하면 필요한 화면으로 이동하며 (사용자 클릭 필요), 화면이 곧 "해야 할 일 목록" 역할을 한다.
...
**⚠️ M8: Home 화면 카드 최대 8개 규칙과 TaskCard 무제한 모순 해결 (StudentTaskCard는 학생용 별칭):**
...
home_card_limit = {
  ...
  per_group_max_cards: {
    ...
    STUDENT_TASKS: 3,  // TaskCard 그룹 최대 3개 (무제한 아님, StudentTaskCard는 학생용 별칭)
    CLASSES: 2,        // 오늘의 반 그룹 최대 2개
    BILLING: 2,        // 수납 요약 그룹 최대 2개
    STATS: 1           // 통계 그룹 최대 1개
  },

  // TaskCard는 무제한 생성 가능하나, 홈 화면에는 우선순위 상위 3개만 표시 (StudentTaskCard는 학생용 별칭)
  student_task_card_display_rule: {
    max_display: 3,  // 홈 화면에 최대 3개만 표시
    overflow_action: 'show_more_button',  // 나머지는 "더 보기" 버튼으로 접근
    overflow_url: '/students/tasks'  // 전체 TaskCard 목록 페이지 (StudentTaskCard는 학생용 별칭)
  },
...
- YYYY-MM-DD: Critical 이슈 수정 완료
  ...
  - AI 인사이트 엔티티 단일화 (ai_insights 정본, MV는 파생물로만 정의)
