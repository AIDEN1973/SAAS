...
AI 출석 예측값은 초기 상태에서만 추천값으로 표시되며, Teacher의 모든 수동 입력은 AI 예측값을 즉시 override한다.
...
**AI가 출석을 "예측"하여 초기 추천값 제공**
- 과거 출결 패턴 기반으로 오늘 출석 예상 학생 추천
- 선생님이 확인/수정만 하면 됨

**⚠️ AI 출석 예측값 적용 범위 명확화:**

AI 예측 출석 체크는 teacher가 수동으로 언제든 수정 가능하며, AI 추천값은 "초기 상태"에만 표시되고, 사용자 입력이 발생하면 AI 데이터는 override 된다.
...
**적용 범위:**
- AI 예측값은 화면 초기 로드 시에만 초기 추천값으로 표시
...
- Fingerprint 불일치 시: `attendance.device_fingerprint_fallback_policy`에 따라 처리 (저장 위치는 tenant_settings(key='config').value(JSONB))
...
- 보안 등급을 'low'로 자동 설정
- `attendance.device_fingerprint_fallback_policy`에 따라 처리 (저장 위치는 tenant_settings(key='config').value(JSONB)):
  - `sms_auth`: SMS 인증 요청
  - `block`: 출결 거부
  - `warn`: 경고 후 출결 허용
...
```json
{
  "attendance": {
    ...
    "device_fingerprint_strict": false, // Device Fingerprint 엄격 모드 (Default Policy: 테넌트 생성 시 false로 설정값으로 저장, low-assurance indicator이므로 기본은 느슨하게)
